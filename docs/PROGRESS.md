...
- [ ] Bottom navigation (`HodithNavHost.kt`'s `NavigationBar`/`NavigationBarItem`): the mockup's active tab gets an `IconHalo`-style glow behind its icon (`.navitem.on .ico`). This is a **shared, app-wide component** rendered on every screen regardless of which screen is "active" in this checklist — not scoped to any single item above, so branch it on `LocalCardDecorationStyle` in its own pass rather than folding it into whichever screen happens to be worked on next. Spotted alongside the Big Picture ring pass; wasn't in the original checklist.
