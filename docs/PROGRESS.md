...
- [ ] **Phase 2** — Big Picture view. Hardest UI problem (custom Compose, shared time axis, pinch-zoom, performance with years of data) — done early so its constraints surface before other screens depend on it.
  ...
  - **Remaining for Phase 2 close-out:**
    ...
    - **Debug-only seed data mechanism — design decided, nothing built yet (no files created).** Picking this up fresh needs this context:
      - **Why not a `BuildConfig.DEBUG` runtime check:** `isMinifyEnabled = false` on the release build type, so R8 wouldn't strip dead code anyway — a runtime check would still ship the seed logic and content inside the release APK. "Fenced out" needs to mean structurally excluded (never compiled into release), not just runtime-gated.
      - **Chosen approach: `app/src/debug/` source set + Hilt `@Multibinds`,** so `main`/release never reference anything debug-only:
        - `main`: a generic `AppInitializer` interface (`fun init()`) alongside `HodithApplication`/`MainActivity`, plus `di/AppInitializerModule.kt` — `@Module @InstallIn(SingletonComponent::class) interface` with `@Multibinds fun appInitializers(): Set<AppInitializer>` (guarantees a valid, possibly-empty set to inject even with zero contributions). `HodithApplication` gets `@Inject lateinit var appInitializers: Set<@JvmSuppressWildcards AppInitializer>`, and `onCreate()` calls `super.onCreate()` then `appInitializers.forEach { it.init() }`.
        - `app/src/debug/kotlin/com/secondmonday/hodith/debug/` (new source set, doesn't exist yet): `SeedDataInitializer` (`@Inject constructor(repository: HodithRepository, clock: Clock) : AppInitializer`) — idempotent (checks `repository.observeActiveCases().first().isNotEmpty()` before inserting anything), runs on `CoroutineScope(SupervisorJob() + Dispatchers.IO)`; `SeedDataModule` binds it with `@Binds @IntoSet`.
        - Seed content sketch: ~6 `CaseEntity` specs spanning up to ~380 days, mixing `durationMode` (NONE / START_STOP), `intensityEnabled` cases (reuse `MIN_INTENSITY`/`MAX_INTENSITY` from `domain/timeline/TimelineLayout.kt`, don't reinvent), and varied event density (bursty vs. sparse) — enough range to exercise pinch-zoom/scroll/density across all four `ZoomLevel`s. Deterministic via a seeded `Random`.
      - **Verification note for whoever builds this:** run the normal three gates, **plus a one-off `./gradlew assembleRelease`** — the whole point of this design is variant-specific compilation (empty `AppInitializer` set in release), so it's worth confirming release still builds clean, not just debug.
