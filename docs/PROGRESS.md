...
- [ ] **Phase 8** — Widgets. In progress on `feature/list-widget` (List widget only; Single-case widget is a later branch).
  ...
  - **To do:** on-device verification of the two actions above; WorkManager periodic refresh for elapsed-time/today-count rollover (dependency added, no `Worker` yet); widget-picker preview image (deferred to the existing app-icon ship-checklist item); cleanup pass.
