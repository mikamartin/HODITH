...
- [x] **Phase 9** — Triggers + check-ins (WorkManager, notifications). Design simplified from the original spec during Phase 9 planning: per-Case check-in *custom days* is dropped, since a `SILENT_FOR` Trigger already covers "notify me after N days of silence on this specific Case" — Case Edit's check-in control shrinks from a DEFAULT/CUSTOM/OFF segmented picker down to a single on/off toggle, and the only days-interval configuration left is Settings' app-level default. The hunch-derived default (2×expected-gap, spec §11) still applies automatically per-Case when a Hunch exists — that's engine logic, not a manual setting, so it's unaffected.
  - **Branches, in dependency order:**
    ...
    5. [x] `feature/notification-infra` — `NotificationEvaluator`/`Notifier`/`SystemNotifier`/`NotificationChannels`/`NotificationEvalWorker` (~6h periodic) evaluate Triggers/check-ins against real data and post minimal notifications. `RoomHodithRepository` triggers immediate per-Case evaluation on event mutations. `POST_NOTIFICATIONS` requested once via `NotificationPermissionRequestSignal`; Home's new `NotificationsDeniedBanner` shown when denied. New Voice `Notifications` section.
    6. [x] `feature/notification-actions` — new `NotificationActionReceiver` handles check-in notifications' **Log** (respects `logFlow`, reusing `WidgetLogTrampolineActivity` for `DETAIL_SHEET`) and **All quiet** (re-arms `lastCheckInAt`) actions. Trigger/check-in notifications now deep-link to the Case's detail screen; the multi-case summary opens Home. Re-arming is explicit-only now, so an ignored check-in recurs each periodic pass instead of firing once per interval; `evaluateAll` collapses 2+ due check-ins into one summary notification. New Voice action/summary keys, all three voices.
- [ ] **Phase 10** — Share cards, export/import, Settings polish (About).
  - **Branches, in dependency order:**
    1. [x] `feature/export-import` — Settings export/import JSON (Moshi, `schemaVersion: 1`), import validates before touching the DB and is all-or-nothing (spec §16); error handling for malformed/incompatible/unreadable files plus user-facing success/failure feedback for both actions. Room entities carry `@JsonClass` directly (no DTO layer); import is a full replace inside one `withTransaction`, FK-ordered, so ids never need remapping. `SettingsViewModel`'s real logic (`performExport`/`performImport`) is unit-tested via a `BackupFileWriter` seam; the `ContentResolver`-touching wrappers are untested framework glue by design. Scope is Room data only — Settings prefs (theme, check-in default) are deliberately excluded.
    2. [x] `feature/about-screen` — wiring only: About screen reached from Settings, real navigation, placeholder copy for version/privacy statement/licenses (spec §14, §16). Actual content (final copy, hosted privacy policy link) is a Phase 12 follow-up, not this branch.
    3. [ ] `feature/share-cards` — spec §13. Mockup/Compose Preview pass first for all three theme templates (story 1080×1920 + square 1080×1080), validated before building the real pipeline; then the Share preview screen (story/square toggle, editable display name, section toggles — notes/tags never included), offscreen Compose→bitmap render, FileProvider, system share sheet, entry point on Case Detail header.
...
- [ ] **Phase 12** — Leftover work deferred from earlier phases, picked up opportunistically once the phases above land:
  ...
  - [ ] About screen content polish (deferred from Phase 10's `feature/about-screen`, which only wires the screen): real copy for version/privacy statement/licenses, and a privacy policy hosted on the SecondMonday Studios website, linked from the screen instead of inline placeholder text.
  ...
  - [ ] Two cosmetic build-time warnings noticed during Phase 10's `feature/export-import`, neither worth chasing on its own: a "Kapt support in Moshi Kotlin Code Gen is deprecated" notice from `hiltJavaCompileDebug` even though Moshi is wired in via `ksp()` only (KSP-generated adapters work correctly; likely Moshi's processor warning whenever it's discoverable on any annotation-processing-adjacent classpath, regardless of which mechanism invokes it); and a K2 "`@ApplicationContext` applied to value parameter only" forward-compat notice (KT-73255), already present on `Notifier.kt`/`WidgetRefresher.kt` before that branch. Worth a look if either ever escalates from warning to error on a future Kotlin/Hilt bump.
