...
- [ ] **Phase 8** — Widgets. In progress on `feature/list-widget` (List widget only; Single-case widget is a later branch).
  - **Done:** `ListWidget` reads real pinned/active Cases (reuses `homeCaseRows`); one-tap log and Start/Stop actions; `WidgetLogTrampolineActivity` for `DETAIL_SHEET` Cases (hosts the real `LogDetailSheet`, real theme/voice); `ListWidgetConfigureActivity` prompts a Case picker only the first time nothing is pinned yet, otherwise skips through; `WidgetRefresher` so pinned/archived changes anywhere in the app refresh the widget. Widget chrome (palette + copy) is fixed/`PlainVoice` regardless of in-app theme — deliberate. `ListWidgetConfigureViewModel` and `WidgetLogSheetViewModel` now have unit test coverage; the latter's `save()` was switched from a direct Glance `updateAll` call to the injected `WidgetRefresher` (matching `ArchivedCasesViewModel`/`CaseEditViewModel`) so it can run on the JVM. `WidgetRefreshWorker` now enqueues a 15-minute periodic refresh from `HodithApplication.onCreate()` so elapsed-time/today's-count don't go stale between explicit refreshes; it resolves `WidgetRefresher` via a Hilt `EntryPoint` at `doWork()` time rather than constructor injection, since `Configuration.Provider` conflicts with `HiltTestApplication` in instrumented tests. Covered by the repo's first `@HiltAndroidTest` (scoped to that one test file).
  - **Verified on-device:** real data + resize, the configure/picker flow with persistence, the `DETAIL_SHEET` trampoline saving a real event.
  - **Not yet verified on-device:** one-tap `QuickLogAction` and `StopEventAction` taps (paused mid-session to conserve emulator resources).
  - **To do:** on-device verification of the two actions above; cleanup pass.
