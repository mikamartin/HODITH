...
- [ ] **Phase 9** — Triggers + check-ins (WorkManager, notifications). Design simplified from the original spec during Phase 9 planning: per-Case check-in *custom days* is dropped, since a `SILENT_FOR` Trigger already covers "notify me after N days of silence on this specific Case" — Case Edit's check-in control shrinks from a DEFAULT/CUSTOM/OFF segmented picker down to a single on/off toggle, and the only days-interval configuration left is Settings' app-level default. The hunch-derived default (2×expected-gap, spec §11) still applies automatically per-Case when a Hunch exists — that's engine logic, not a manual setting, so it's unaffected. Triggers are new end-to-end: no entity, no engine, no screen exist yet, so they're the bulk of this phase's work.
  - **Branches, in dependency order:**
    ...
    5. [x] `feature/notification-infra` — `domain/TriggerEngine.kt`/`CheckIn.kt`'s evaluation functions widened from `internal` to public so a new `notification/` package can call them: `NotificationEvaluator` (orchestrates both engines against real data), `Notifier`/`SystemNotifier`, `NotificationChannels`, `NotificationEvalWorker` (~6h periodic, same plain-constructor + Hilt `@EntryPoint` pattern as `WidgetRefreshWorker`). `RoomHodithRepository`'s event mutations now trigger immediate per-Case evaluation, fire-and-forget on a new app-scoped `CoroutineScope`. `POST_NOTIFICATIONS` requested once via a new `NotificationPermissionRequestSignal`, collected at the app root rather than per-screen (Case Edit navigates away the instant a save completes, which could dispose a per-screen listener first). Home's new `NotificationsDeniedBanner` shown when denied — extracted a shared `ActionBanner` alongside the existing `StaleOngoingBanner`. New Voice `Notifications` section. Content is intentionally minimal (title/body, tap opens the app — no deep-link or Log/All-quiet actions); that's the next branch.
