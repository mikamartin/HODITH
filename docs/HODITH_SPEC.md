...
- The event list is newest-first by start. A Case that tracks duration (`durationMode ≠ NONE`) also gets a **Started / Ended** sort toggle on the Log tab: "Ended" lists still-running events first, then finished events by most recent end. It's a device-level view preference (`SettingsRepository`/DataStore, like the theme) applied across every Case, not a per-Case schema field.
