...
- Intensity is not encoded on the grid. Duration is, but only for an event whose active span crosses a calendar-day boundary: its icon then appears on every day it covered, the start day's icon carrying a `primary` ring, and a still-running event trails to today. A same-day duration event is indistinguishable from a moment event here. A `NONE` Case shows no spans at all — every icon is a single-day mark regardless of any stored `endedAt` (§9 active span). Tapping a day still opens its events; on a spanned day the row reads "ongoing since …" / "lasted …" rather than a clock time that would mislead. Intensity, and duration finer than a day, live in a case's own stats (§10).
...
| Screen | Contents |
|---|---|
| **Home** | Case list (drag to reorder): icon, name, today/this-week count (a duration event counts while its active span is still open into the window, matching the calendar heatmap — §9), quick-log button, ongoing indicator. FAB: new Case. Trigger banners if notifications are denied. Text link to **Archived Cases**, shown only once at least one Case is archived. |
