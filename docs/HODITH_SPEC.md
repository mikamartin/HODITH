...
- Intensity is not encoded on the grid. Duration is, but only for an event whose active span crosses a calendar-day boundary: its icon then appears on every day it covered, the start day's icon carrying a `primary` ring, and a still-running event trails to today. A same-day duration event is indistinguishable from a moment event here. Tapping a day still opens its events; on a spanned day the row reads "ongoing since …" / "lasted …" rather than a clock time that would mislead. Intensity, and duration finer than a day, live in a case's own stats (§10).
...
The duration and intensity cards are gated purely on the Case's current `durationMode`/`intensityEnabled` flags — turning either off hides its card but keeps every event's recorded `endedAt`/`intensity` untouched, so turning it back on restores the card with all its history intact.
