...
- Widget type ramp, spacing, and shape are hand-set `WidgetCommon.kt` tokens, not a shared import from `ui/theme/Type.kt`/`Shape.kt` — Glance can't consume M3's `Typography`/`Shapes` directly. Two structural gaps this leaves permanently unclosed: `androidx.glance.text.TextStyle` has no `lineHeight`/`letterSpacing` property, and its `FontWeight` only defines Normal/Medium/Bold (no 600/SemiBold, which Plain's `headlineSmall`/`labelSmall` roles use in-app — the widget header takes Bold as the nearest rung, see `WidgetHeaderTitleWeight` below). Values below were chosen to match Home's Plain rendering as closely as Glance's API allows, and a unit test (`WidgetTokenFidelityTest`) fails if `Type.kt`/`Shape.kt`'s Plain values — or the `HomeScreen.kt` plank layout values the layout tokens mirror — are later changed without updating these.

  | Widget token | Value | Home / `Type.kt`-`Shape.kt` role |
  |---|---|---|
  | `WidgetCornerRadius` | 8dp | `Shape.kt` `plainShapes.medium`/`large`; also the Plain plank `Card`'s default M3 shape (`HomeScreen.kt`'s `PlainPlankHomeCaseListItem`) — the List widget applies it per Case row so each reads as a discrete plank |
  | `WidgetIconGlyphSize` | 24sp | `MaterialTheme.typography.headlineSmall` (Home row icon) |
  | `WidgetCaseNameSize` | 16sp | `MaterialTheme.typography.titleMedium` (Home row name) — kept at `FontWeight.Medium` like the app; Glance renders it a touch lighter than Home's body-font Medium but that's within the palette-vs-type fidelity budget |
  ...
  | `WidgetHeaderTitleSize` | 14sp | Size is widget-only (Home's header is `headlineSmall` = 24sp; a widget canvas keeps it compact), mapped to `titleSmall` by purpose |
  | `WidgetHeaderTitleWeight` | `FontWeight.Bold` | Home's header `Text` renders at `headlineSmall`, which carries Plain's display weight `FontWeight.SemiBold`; Glance has no 600 rung, so Bold is the nearest. A weight-only nod to the app role (the size stays compact, above). `WidgetTokenFidelityTest` fails if Plain's `headlineSmall` weight moves off SemiBold |
  | `WidgetInfoMessageSize` | 14sp | Widget-only, no Home equivalent — mapped to the nearest M3 rung by purpose, `bodyMedium` |
  | `WidgetOuterPadding`, `WidgetRowPadding`, `WidgetLogButtonSpacing`, `WidgetSingleCaseOuterPadding`, `WidgetSingleCaseLogButtonSpacing`, `WidgetHeaderPadding` | 12dp / 10dp / 8dp / 8dp / 6dp / 15dp | Widget-only layout, no Home analogue — a widget's canvas is far narrower than a phone screen, so these stay hand-tuned. `WidgetHeaderPadding` keeps the tap target past `MinTapTarget` for the List widget header and empty-state message — both bare `.fillMaxWidth()` `Text`s (no wrapping `Box`, no fixed height), so the `Text` wraps and grows instead of being clipped |
  | `WidgetPlankSpacing` | 8dp | Transparent Spacer between List widget Case planks and below the header, so each row reads as a discrete card — 2× `HomeScreen.kt`'s `PlainPlankVerticalMargin` (the margin Home's plank `Card` carries above *and* below). `WidgetTokenFidelityTest` locks the 2× relationship. `WidgetCornerRadius` (above) rounds each row's surface |
- Two Glance layout facts the widgets are built around: (1) a `Text` wrapped in a `Box` is measured at its natural single-line width even when the `Box` is `fillMaxWidth()`, so a long string clips at the edge instead of wrapping — put `.fillMaxWidth()` on the `Text` itself (its `TextView` then gets `match_parent` and wraps). (2) There is no margin: every `padding` is interior and `background` fills it, so a gap *between* siblings (e.g. between Case planks) has to be a real transparent `Spacer`, not padding on either sibling.
