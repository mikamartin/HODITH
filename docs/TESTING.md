...
| **ViewModels** | State reduction, undo window; `ongoingEventsIn`/`ongoingEventIn` (earliest-first, empty for non-`START_STOP`) and Home rows' `runningCount`; reopen round-trip (`planSaveEvent` clears `endedAt` and rebases `staleNudgeDismissedAt` only on a genuine reopen); Case Edit's required-name, max-length (name/description), case-insensitive duplicate-name-among-active-Cases validation (self-name exempt when editing), and the leave-`START_STOP`-while-running confirm (held mode change, confirm stops every open event on save, cancel reverts, non-`START_STOP` never guarded) |
...
| Widgets — chrome & trampoline | List widget title/empty-state tap and Single-case widget's "Case is gone" tap all open `MainActivity`, driven via a real `AppWidgetHost` (`WidgetChromeNavigationTest`) — everything *inside* the List widget's `LazyColumn` row still needs the manual pass (see MANUAL_TEST_PLAN.md's Widgets section for why); the `DETAIL_SHEET` trampoline sheet (`WidgetLogTrampolineActivityTest`) saves a real event and finishes, driven directly since the row that launches it is inside that same unreachable `LazyColumn`; the Single-case widget's "N running" pill (`WidgetActionsFlowTest`) replaces the Stop button and opens `MainActivity` with the Case id set |
