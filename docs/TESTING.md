...
| **Export/import** | Round-trip equality; newer schema versions rejected, older ones tolerated via an upgrade-chain mechanism (currently empty - no version older than current has ever shipped); malformed JSON is all-or-nothing (DB untouched on failure); import of an ongoing event |
