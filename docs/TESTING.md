...
| Area | What to cover |
|---|---|
| Room DAOs | CRUD per entity, cascade delete case → events/hunches/triggers, delete-all-cases (and that it doesn't on its own remove the case-independent `tags` table), delete-all-archived-cases (scoped to archived, cascades the same as per-case delete, leaves active cases untouched), tag join queries, "events in window" queries, all-cases-with-events query feeding the Big Picture and its tag-nesting variant, backup export/import round-trip and its all-or-nothing rollback on a mid-transaction failure |
