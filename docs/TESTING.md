...
| **Notification evaluation** | `NotificationEvaluator` orchestration against fakes: fires a Trigger/check-in only when due and persists `armed`/`lastFiredAt`/`lastCheckInAt` back; skips archived Cases and Cases with `checkInsEnabled = false`; unknown Case id is a no-op; `evaluateAll` spans every enabled Trigger and every active Case, not just one |
