# HODIT — Product Specification

**HODIT: How Often Does That Happen** — a local-only Android app that answers one question: how often does that *actually* happen?
...
Some of these you influence, many you don't. HODIT doesn't care — it just counts, honestly. The audience is deliberately broad for now: anyone who's ever said "this always happens" and wondered.
...
- **HODIT is not a habit tracker.** No streaks, points, scores, rewards, or reminders to "do the thing" — Cases are often about events nobody controls. If a feature idea pushes toward behaviour change rather than observation, it doesn't belong.
...
- HODIT branding on the card is a small, unobtrusive footer ("counted with HODIT") — honest attribution, not an ad.
...
| **Settings** | Theme/voice picker with live preview card, default check-in interval (off / 7 / 14 / 30 days), export/import JSON, About. |
...
| Architecture | MVVM — ViewModel + StateFlow; `HoditRepository` single source of truth |
...
Suggested package: `com.secondmonday.hodit`.
