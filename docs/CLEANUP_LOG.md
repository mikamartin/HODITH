...
**Deferred:**
- **Prototyping Android's auto-generated group summary as a simpler replacement** for the app-managed one — needs a real device across 1/2/3 grouped notifications to confirm one-sound + no-renag + no silent-lone-notification. Kept the app-managed summary; revisit only if the maintenance cost bites.
- **No dedicated `NotificationActionReceiverTest` for the post-All-quiet summary refresh.** Its three existing tests already exercise the new `WidgetEntryPoint.notifier()` / `settingsRepository()` accessors and the `refreshGroupSummary` call by construction (they'd fail to resolve otherwise), and the collapse logic itself is covered by `NotifierContentTest.cancelCheckIn_droppingToOneChild_removesTheSummary`. A receiver-level duplicate would need two real cases sharing device state — flaky for little gain.
...
- Instrumented (compile-checked here; **not run locally — no device/emulator this session**; CI's `instrumented-tests.yml` runs them on the PR): `DatabaseFreshInstallTest` gains a v6→v7 `MigrationTestHelper` test (column gone, rows survive). `NotifierContentTest` — the summary test → `twoDueCheckIns_shareTheGroup_andGetAnAlertOnceSummary` (group key, `GROUP_ALERT_SUMMARY`, `FLAG_GROUP_SUMMARY`, `FLAG_ONLY_ALERT_ONCE`) + `cancelCheckIn_droppingToOneChild_removesTheSummary`; new `notifyCheckInDue_setsOnlyAlertOnce…`. `HomeScreenTest` / `CaseDetailScreenTest` / `CaseDetailInsightsTabTest` — stale-banner tests deleted, harness `onDismissStalePrompt` params dropped.
