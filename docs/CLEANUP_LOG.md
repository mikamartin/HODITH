...
- **`SettingsScreenTest.deleteAllData_opensConfirmDialog_confirmInvokesCallback` / `_cancelDoesNotInvokeCallback` regressed** — the full instrumented run (below) caught them. Both click `settingsDeleteAllDataButton` with no `performScrollTo()`; that row is the last item of the last plank, and the new `TimeFormatSection` (one section-height, in the Appearance plank above) pushed it below the default test window, so the click no-op'd and the confirm dialog never opened. Added `performScrollTo()` before the click in both, matching `loadDemoData_tapInvokesCallback` in the same file. Re-verified: `SettingsScreenTest` 23/23 green on the emulator, then the full suite 234/234.
...
**Verified:** `./gradlew clean` then `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green (a one-off `lintAnalyze*` "bug in lint" failure on unrelated `backup/*Test` files, from stale KSP output after the emulator-crash churn, cleared by `clean`). Full `connectedDebugAndroidTest` on `emulator-5554` (`ANDROID_SERIAL`-scoped; a physical Pixel 8 also attached — DEV_PLAYBOOK §5), 234/234 green after the `deleteAllData_*` fix. (First full run flagged those 2; a mid-suite `INSTRUMENTATION_ABORTED: System has crashed` on a follow-up run was emulator instability — cleared by a cold reboot, unrelated to the diff.) The later `insightsFrequencyWeekAxisLabel` Voice fix re-verified: `CaseDetailInsightsTabTest` 21/21 on the emulator, `VoiceTest` + `EventTimeFormatTest` green.
