...
- Instrumented (compile-checked here; **not run locally — no device/emulator this session**; CI's `instrumented-tests.yml` runs them on the PR): `DatabaseFreshInstallTest` gains a v6→v7 `MigrationTestHelper` test (column gone, rows survive). `NotifierContentTest` — the summary test → `twoDueCheckIns_shareTheGroup_andGetAnAlertOnceSummary` (group key, `GROUP_ALERT_SUMMARY`, `FLAG_GROUP_SUMMARY`, `FLAG_ONLY_ALERT_ONCE`) + `cancelCheckIn_droppingToOneChild_removesTheSummary`; new `notifyCheckInDue_setsOnlyAlertOnce…`. `NotificationActionReceiverTest` gains `actionAllQuiet_collapsesTheGroupSummary_whenOneCheckInRemains` — posts two real check-ins via the injected `Notifier`, broadcasts All quiet for one with its real `checkInNotificationId` (made `internal` for this), asserts the receiver's `refreshGroupSummary()` drops the now-redundant summary while the other check-in stays; `insertedCaseId` → a tracked list, `cancelAll()` added to teardown. `HomeScreenTest` / `CaseDetailScreenTest` / `CaseDetailInsightsTabTest` — stale-banner tests deleted, harness `onDismissStalePrompt` params dropped.

**Verified:** `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green; full unit suite 574/0. `compileDebugAndroidTestKotlin` green. Instrumented tests not executed this session (no device) — they build, and CI runs them on the PR.
