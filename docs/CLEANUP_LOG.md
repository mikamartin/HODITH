...
- **Deleted** `viewmodel/LogSort.kt` and `LogSortTest.kt` — `sortEventsForLog`'s logic is superseded by the SQL queries; its ordering-contract test cases moved to `EventDaoTest`.
...
- *Tests* — `EventDaoTest` (6 new cases: cap + id-tiebreak for `PagedByStart`; running-floats, non-`START_STOP` ignores running-state, `MANUAL` end-less fallback, and a cap check for `PagedByEnd`), `FakeHodithRepositoryTest` (cap/`hasMore`/boundary/BY_END-gating), `CaseDetailViewModelTest` (window caps at 30, grows by 50, sort-order reset, `hasMore`), `CaseDetailScreenTest` (Show More hidden/shown/tap, a `logEvents`-vs-full-`events` regression guard, the existing "tap Ended" test reworked to assert the callback rather than re-prove reordering now that the screen is purely a renderer of what the ViewModel already sorted) — plus a two-line fix to `CaseDetailInsightsTabTest`'s direct `CaseDetailScreen(...)` call site, the one other place in the repo besides `CaseDetailScreenTest` that constructs the screen directly, for the two new required callback params. `connectedDebugAndroidTest` run scoped to the touched classes on `Pixel_8_API36(AVD)`: `EventDaoTest` 23/23, `CaseDetailScreenTest` + `CaseDetailInsightsTabTest` 61/61, both clean. `ktlintCheck → lintDebug → test → assembleDebug` all green.
...
**Verified:** `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green. `connectedDebugAndroidTest` scoped to `EventDaoTest`, `CaseDetailScreenTest`, and `CaseDetailInsightsTabTest` — all green on `Pixel_8_API36(AVD)`.
...
- **Lean projections for Home / the widgets.** New `EventDao.observeActiveCaseEventSpans()` — a flat `events JOIN cases` returning `CaseEventSpan(caseId, occurredAt, endedAt, durationMode)` (new `data/` class), no `@Relation`, no full-row hydration — and `EventDao.observeOpenEvents()` for the ongoing indicator (small result set). New `homeCaseRowsFlow(repository, now)` combines those two + `observeActiveCases()` and maps through `homeCaseRows`; `HomeViewModel` and both Glance widgets all build their rows through it (widgets filter the result to their own Cases). `homeCaseRows` now takes `(cases, eventSpans, openEvents, now)`, grouping the projections by `caseId`. `HomeViewModel.uiState` maps on `.flowOn(Dispatchers.Default).conflate()`. Rapid-logging burst measured **~405 ms/insert + device ANR → ~1.9 ms/insert, no ANR**; cold first-emission ~unchanged (a *slow* refetch starving the SQLite writer was the ANR cause, not the query's own cost).
- **`CaseDao.observeArchivedCaseCount()`** (`SELECT COUNT(*) FROM cases WHERE archived = 1`) replaces `observeArchivedCasesWithEvents().map { it.size }` on Home. ~83 ms → ~3 ms, and no more materialising every archived event continuously. `ArchivedCasesViewModel` left on the full `@Relation` — the Archived screen is rarely open and archived Cases are few.
- **`remember(...)` around `insightsTabState` / `hunchTabState`** in `CaseDetailScreen.kt`, keyed on the real inputs. Stops the many-pass aggregation firing on unrelated recompositions. `now` kept as a key: the recompute is a few ms and de-ticking risks a day-boundary regression (trend windows, running-event span end, heatmap "today") for no felt gain.
- **`activeSpanEnd` gained a primitive overload** (`occurredAt, endedAt, durationMode, now`) so the span-end rule lives in one place; the `EventEntity` overload delegates to it.
- **Dead code removed** — `CaseDao.observeActiveCasesWithEvents()` (the *active* `CaseWithEvents` `@Relation`; the archived and `AndTags` variants stay) lost its last caller. Dropped from `CaseDao` / `HodithRepository` / `RoomHodithRepository` / `FakeHodithRepository`; `CaseWithEventsTest` (3) deleted, its `FakeHodithRepositoryTest` case removed. The plain-relation nesting is still covered by `CaseDaoTest.observeArchivedCasesWithEvents_…`; the tag variant by `CaseWithEventsAndTagsTest`.

**Checklist walk (against the staged `git diff`):**

- *Duplication* — **found:** rerouting the widgets first left the 3-flow `combine` duplicated (and larger) in `ListWidget` + `SingleCaseWidget`. **Fixed:** extracted `homeCaseRowsFlow`, so both widgets and `HomeViewModel` share one wiring (each filters the output). The new DAO queries are SQL-pushdown *of* Kotlin work, not new duplication. No repeated styling / inline strings / duplicated ViewModel logic.
- *Decoupling* — `homeCaseRows` / `homeCaseRowsFlow` take a `HodithRepository` and a `() -> Long`, no `Clock`, no `android.*`; `CaseEventSpan` is a `data/` type. `ActiveSpan.kt` stays pure Kotlin. No new UI types in the ViewModel. `flowOn(Dispatchers.Default)` is on the ViewModel flow, not the repository.
- *Complexity & pattern health* — `HomeViewModel.uiState` is a 3-arg `combine` again (was going to 5). `remember` vs `rememberSaveable`: `remember` is right for recomputable derived state. `produceState` keys unchanged (selection); widgets still capture `now` once, as before. No composable newly over-long.
- *Dead code & hygiene* — **found:** several code comments cited `PROGRESS.md F1/F3/F5`, which get removed as items land. **Fixed:** reworded to describe the reason or cite the spec. Throwaway probes built, run, deleted — `git status` clean of them. `observeActiveCasesWithEvents` + tests removed (above). No commented-out code, no unused imports (ktlint clean).
- *Repo hygiene* — `git status` = `git diff --name-only`; two new files are LF in the working tree, normalised to LF in the blob by `core.autocrlf` like every committed `.kt`. No secrets, no `C:\…` paths in any committed file (the baseline notes are referred to only as "local notes").
- *Naming* — `CaseEventSpan`, `homeCaseRowsFlow`, `observeActiveCaseEventSpans`, `observeOpenEvents`, `observeArchivedCaseCount` follow the `observe*` / projection-type conventions; files in the right packages.
- *Hardcoded values / accessibility / deprecated APIs* — n/a: no constant, colour, tappable target, or deprecated call touched; lint clean.
- *Spec review* — §9/§14 count semantics and §15 widget behaviour are byte-identical (the count math reads a projection instead of full events, same rule via the same `activeSpanEnd`). `HomeViewModelMappingTest`'s boundary cases are unchanged. Nothing to update.
- *Tests* — new `HomeCountQueriesTest` (4, one `@Smoke`, correctly not `@UiTest`) for the real queries; `FakeHodithRepositoryTest` +3 for the fakes; `HomeViewModelMappingTest` keeps all its boundary assertions via a thin `homeRows` bridge helper + one new case for the widget filter path. `activeSpanEnd`'s primitive overload is covered transitively (the `EventEntity` overload passes straight through) and by the 21 `homeCaseRows` cases — a direct duplicate test would not earn its keep. **Instrumented `data` (69) + `widget` (17) + Home/CaseDetail/Archived UI ran green earlier this session, but *before* the dead-code removal and the `homeCaseRowsFlow` extraction — a re-run is pending (the local emulator crashed). The remaining delta is a method + test-file deletion the compiler confirms is unreferenced, plus the widget wiring extraction covered by the green JVM suite.** No new system-boundary flow → no MANUAL_TEST_PLAN change.

**Also:** softened DEV_PLAYBOOK §5's "88/88 … tests passing" to drop the self-updating count (this diff shifts the repository-shard total).

**Deferred:** nothing.

**Docs updated:** `PROGRESS.md` — F1/F3/F5 removed (done), Performance section rewritten to the open items with no finished-work / branch-name references; stale S3 (segmented-row crowding, not reproducible) + its cross-refs removed. `DEV_PLAYBOOK.md` — new §8: a "keep in mind" note on Room's table-level invalidation, the lean-query rules to follow, and the throwaway-probe recipe (no PROGRESS references, no finished-work narrative). `TESTING.md` — Room DAOs row (lean projections; the removed `CaseWithEventsTest`), ViewModels row (`homeCaseRows`' inputs). SPEC untouched.

**Verified:** `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green (646 JVM tests). Instrumented re-run pending the emulator.
...
**Found & fixed (checklist walk-through against the real `git diff`):**
- **`eventDetailSummary(EventEntity, …)` refactored to delegate to a primitive-parameter overload** (`occurredAt`/`endedAt`/`intensity`/`note`/`tagNames`) — Big Picture holds only its `CalendarEvent` projection, not an `EventEntity`, so rather than drag entity types into the projection or hand-roll the ` · `-join + duration/intensity Voice copy a third time, the shared join logic moved down one level. The Insights path is unchanged (still calls the entity overload). Covered directly by three new `CaseDetailFormattingTest` cases.
- **`ToggleRow` extracted from `SharePreviewScreen.kt` to `ui/common/ToggleRow.kt`** — it was `private` to Share; the detail dialog needed the identical label+`Switch` row (`Modifier.toggleable`, `Role.Switch`, merged a11y node). Share now imports it; its four now-unused imports (`Row`, `Switch`, `toggleable`, `Role`) removed, trailing blank line trimmed (ktlint).
- **`BigPictureViewModel` constructor gained `SettingsRepository`** — threaded as `combine(observeActiveCasesWithEventsAndTags(), observeBigPictureDetail())`; `bigPictureUiState` stays a pure function with a defaulted `detail` param so its ~8 existing call sites/tests compile untouched. `setDetail` reads the current value via `observeBigPictureDetail().first()` before flipping one field — no local mirror of the preference in the VM.
- **`CalendarEvent` gained `intensity: Int?`** only; `endedAt` already survives for a same-day duration event on a tracking Case (only multi-day detection reads it), so Duration needed no projection change — pinned by a new `bigPictureUiState` test.
- **Per-event tag pills now sort alphabetically** — `bigPictureUiState` builds `CalendarEvent.tags` from the DB relation, which returns them in attach order, so a row's pills were in an arbitrary order that didn't match the (sorted) filter dialog list. One `.sorted()` at the projection, so the single render path stays dumb; covered by the updated tag-mapping VM test. The Log tab and Insights drill-down rows have the same unsorted-relation quirk via `eventDetailSummary`; **raised with the human and spun out as PROGRESS.md S4** rather than pulled into this branch.
- **`EventDetailRow` layout reworked** — four related fixes, all latent before this branch but surfaced while reviewing the reworked row: (a) the wordy "ongoing since …" / span-range labels wrapped awkwardly inside the name `Row`; they now take their own line; (b) a same-day event's bare clock time, still a second `Text` in that `Row`, got squeezed into an ugly wrap once the Case name was long — icon + name + time are now one `AnnotatedString` in a single `Text` (time a trailing muted `SpanStyle`), so the whole line wraps as a unit; (c) a carried-day ongoing/span label showed only the date ("Ongoing since Jul 21"), dropping the real start time — now `formatSpanDateTime` (new `EventTimeFormat` formatter, "MMM d, h:mm a") gives it date *and* time when the event began on another day, a same-day start still shows just the time; (d) rows sat 12dp apart with flush internal lines — bumped to 20dp between rows with 3dp between a row's own lines.
- **`BigPictureDetail.parse`/`serialize` kept on the type, not inlined into `DataStoreSettingsRepository`** — so the tolerance rules (null → `DEFAULT`, `""` → all-off, unknown/duplicate/mis-cased tokens dropped, order-independent) are JVM-unit-tested with no DataStore; the repo impl stays a one-line `stringPreferencesKey` get/set like every sibling key. `DataStoreSettingsRepository` has no tests today and this didn't add the first (`feedback_test_thin_logic_early` satisfied by the extracted pure logic).

**Sections walked, nothing to do:**
- *Duplication — Voice* — `bigPictureDetailDialogTitle` + `bigPictureDetailEditDescription` authored ×3 (Serious/Goth/Quirky) in this commit; the four field labels (`bigPictureDetailNotesLabel` …) are interface `get()` defaults, identical across voices like `caseDurationModeNone` — `VoiceTest`'s "identical across voices" check excludes non-abstract keys, so no violation. `bigPictureEventNoteEmptyState` fully retired (interface + 3 overrides; grep clean in `app/src`).
- *Decoupling* — `BigPictureDetail` is pure Kotlin in `data/`, no `android.*`; the VM references no UI types; `BigPictureScreen` gets `uiState.detail` + an `onToggleDetail` lambda, not the VM.
- *Complexity* — `EventDetailRow` is now ~110 lines (time-label `when`, the meta line, the heading `AnnotatedString`, then a `Column` of ≤5 conditional `Text`s + the tag flow) — **considered** splitting out the heading/time-label builders, **declined**: each block is a few lines, reads straight top-to-bottom, and every helper would be single-call. Under the ~150 guideline. `FilterSummaryRow` gained the edit `IconButton` + one `InfoDialog`. `showDetailDialog` uses `remember { mutableStateOf(false) }`, matching the `showCasesDialog`/`showTagsDialog` siblings right beside it (none save — a dialog-open flag isn't config-change-critical).
- *Spec Review* — HODITH_SPEC §9's Big-Picture day/week-dialog bullet and the intensity/duration line were the touched sections; both walked and updated for the intentional divergence (the configurable rows). No Future Work §17 item implemented.
- *Tests* — new pure logic (`BigPictureDetail`, the `eventDetailSummary` overload, `formatSpanDateTime`, the VM wiring) all gained JVM coverage; `BigPictureScreenTest` stays one `@UiTest` class with an existing `@Smoke`. No new system-process boundary (DataStore is in-process), so nothing for MANUAL_TEST_PLAN — but the "preference survives an app restart" check is called out below as a human step.
- *Established components* — `InfoDialog`, `ToggleRow`, `IconButton`, `eventDetailSummary` all reused; nothing re-implements M3.
- *Hardcoded values* — the default toggle set is `BigPictureDetail.DEFAULT`, a named value; no product constant inline. `Icons.Filled.Edit` is in `material-icons-core` (already used by `CaseDetailScreen`) — no new dependency.
- *Accessibility* — the edit icon carries `contentDescription = voice.bigPictureDetailEditDescription`; `IconButton` is a 48dp target; `ToggleRow` is a full-width toggleable row.
- *Deprecated APIs* — none; `lintDebug` clean.
- *Naming* — `BigPictureDetail.kt` in `data/` (type-named file, like `TimeFormat.kt`), `ToggleRow.kt` in `ui/common/`, `BigPictureDetailTest.kt` in test `data/`.
- *Repo hygiene* — `git status` clean; no secrets/local paths; the two `sed -i` edits made mid-work were caught converting `BigPictureViewModelTest.kt` to LF and restored to CRLF (repo norm), verified `git diff` carries no line-ending noise (`feedback_sed_crlf_corruption_risk`).
- *Throwaway prototype* — the options prototype is a **published Artifact** (the decision record, linked from the branch/PR), not a repo file; **considered and declined** adding it to `docs/mockups/`, since S8 is actively trying to shrink that directory and the Artifact URL is the durable reference.

**Deferred:** nothing.

**Docs updated:** `HODITH_SPEC.md` §9 — the day/week-dialog bullet now describes the Overview detail preference and its default, the own-line span/ongoing label carrying date + time on a carried day, and notes intensity/sub-day duration stay off the grid but are reachable from the dialog via the toggle (present-tense, no changelog narration). `PROGRESS.md` — S4 removed as resolved; B2's copy-audit list swaps the old "retiring `bigPictureEventNoteEmptyState`" note for the new keys; S8's soft-block and the Standalone batching bullets drop their S4 references. `TESTING.md` — the ViewModels row and the "Compose UI — Big Picture" row extended for the new coverage. No `DEV_PLAYBOOK.md` / `CLAUDE.md` / `README` change.

**Tests:** `BigPictureDetailTest` (new, 14) — round-trip over all 16 combinations, `DEFAULT` pinned, `with` single-field flip, and the parse edge cases. `BigPictureViewModelTest` (+11) — intensity gated on `intensityEnabled`, same-day `endedAt` retained, `detail` on the state, the combine wiring both directions, and `setDetail` flip/round-trip/preserve-others. `CaseDetailFormattingTest` (+3) — the primitive `eventDetailSummary` overload. `EventTimeFormatTest` (+1) — `formatSpanDateTime` in both clock formats and a non-default zone. `BigPictureScreenTest` (+14, `@UiTest`) — placeholder gone, edit-icon opens the switch dialog at the default, toggles report `(field, value)`, icon sits right of the chips, each detail flag governs its row line (day + a week spot-check + a Bright-theme smoke); the four existing span/ongoing assertions updated to the "MMM d, h:mm a" endpoints, and the name/time assertions switched to `substring` matchers now that they share one text node. New Compose `@Preview`s for `EventDetailRow` under three presets and the `FilterSummaryRow` with the edit icon.

**Verified:** `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green (full JVM suite, no failures). Instrumented on a Pixel 8 API 36 emulator, all green: `BigPictureScreenTest` 44/44, the `ui.share` package 12/12 (the extracted `ToggleRow`), the `ui.casedetail` package 57/57 (the `eventDetailSummary` refactor). Still for the human: a manual check that a toggled preference survives an app restart.

## feat/insights-from-first-event

**Scope:** PROGRESS.md item S2 (as renumbered) — the Case Detail Insights tab was withheld until the second event and its placeholder copy was a countdown ("Log N more events to see Insights"), the framing spec §4 warns against. Now: zero events → a flat invitation with no number; one event → the calendar heatmap (one cell), a one-line count note, and the Rhythm and Gaps & streaks cards; Frequency and Trend join from the second event. Mirrors spec §9's existing Big Picture carve-out — an empty calendar doesn't pretend to show a pattern the way an empty chart would. `INSIGHTS_MIN_EVENTS` stays `2`, repurposed from "gates the whole tab" to "gates Frequency + Trend"; the whole-tab gate is now `events.isEmpty()`. The one-event layout and the per-card guarding (Rhythm/Gaps honest at N=1, Frequency/Trend not) were product calls made via AskUserQuestion before building.

**Found & fixed (checklist walk-through against the real `git diff`):**
- **`InsightsTabState.NotEnoughData(eventsRemaining: Int)` → `data object NothingLogged`** — the `eventsRemaining` field existed only to feed the countdown copy; with the copy gone it was dead. `data object` matches the repo convention (`BackupEvent.ExportSuccess`, `SingleCaseWidgetConfigureUiState.Loading`). `Ready.stats` kept non-null (Rhythm/Gaps are always present from one event), so no `?.` churn at the ~25 `as Ready` test sites or in `ShareCardState`.
- **`Voice.insightsNotEnoughDataMessage(Int)` removed** (interface + all three impls); replaced by two no-arg `val`s — `insightsNothingLoggedMessage`, `insightsSingleEventNote` — authored in all three voices in this commit. `VoiceTest` is reflection-driven and re-covers automatically. Names check out against siblings: `…Message` for a zero-state line (`widgetConfigureNoCasesMessage`, `notificationsDeniedBannerMessage`), `…Note` for a muted aside (`hunchTabNoneDataNote`, `bigPictureNoCasesSelectedNote`).
- **Repeated `events.size < INSIGHTS_MIN_EVENTS` in `statsSections`** — extracted to one named `belowStatsMinimum` local (with a comment), shared by the Frequency and Trend guards.
- **Count-note typography aligned with the app's other muted asides** — first drafted `bodyMedium`; the four existing "small note above a data view" sites are `labelSmall` (`bigPictureEarlyDays`, `bigPictureNoCasesSelectedNote`) or `bodySmall` (`hunchTabNoneDataNote`, on the sibling Hunch tab). Changed to `bodySmall` to match the nearest analog, with a comment pointing at it. Not extracted to a shared composable — the four sites vary in both type scale and padding, so a helper wouldn't earn its keep the way `CenteredEmptyState` did (5 identical sites).

**Sections walked, nothing to do:**
- *Decoupling* — verdict/trigger/stats stays pure Kotlin; the `InsightsEngine.kt` change is a KDoc only. The one UI-side threshold check (`state.stats.totalEventCount < INSIGHTS_MIN_EVENTS`, to show the note) reads a domain constant the file's KDoc already referenced and sits beside its existing domain imports (`heatmapLevelFor`, `RHYTHM_TIER_COUNT`, …) — **considered** pushing a `Boolean` onto `Ready` instead, **declined** as more state surface for a one-line display predicate.
- *Complexity* — `InsightsTabContent`'s `Ready` branch grew ~7 lines (one `if` + `Text`); still well under 150.
- *Hardcoded values* — `INSIGHTS_MIN_EVENTS` reused, no new magic numbers; the note's `bodySmall` / `onSurfaceVariant` are theme values.
- *Accessibility* — the count note is informational body text, no tap target; the single heatmap cell keeps its existing tap target and content description.
- *Naming / Voice* — new keys follow the `insights…` prefix, added ×3 in the same commit; folded into B2's copy-audit list.
- *Dead code* — `insightsNotEnoughDataMessage` fully gone (grep clean in `main`); no unused imports (`INSIGHTS_MIN_EVENTS` is now imported and used by `InsightsTab.kt`).
- *Throwaway prototype* — none built; the layout call was a text discussion. `git status` shows only the edited files.
- *Docs — branch-name callout* — the new B2 line ("`feat/insights-from-first-event` added … fold those in") is a branch-name reference the checklist flags as babysitting-prone. **Kept:** it mirrors the pre-existing `feat/declutter-nudges` line beside it, PROGRESS.md is the work tracker (not a current-state doc), and it self-clears when B2 consumes it — the same way the prior cleanup entry deliberately parked this exact key as S2's scope.
- *Repo hygiene* — `git status` clean; no secrets, no local paths; all edits LF, `git diff` is line-level only (no CRLF noise).
- *Deprecated APIs* — none; `lintDebug` clean.

**Deferred:** nothing.

**Docs updated:** `HODITH_SPEC.md` §9 (per-case heatmap renders from the first event, alongside the note and the Rhythm/Gaps cards) and the §10 intro (Frequency and Trend held back until two events) — present-tense statements of current behaviour, no changelog narration. `PROGRESS.md` — S2 removed as resolved; the remaining Standalone items renumbered S3→S2 … S9→S8 with every cross-reference updated (B2's copy-touching list plus a note of the new keys, the batching bullets, S8's mockup-gate list); the new S2 (Trend review) gains a line noting the added event-count guard. `TESTING.md` — the "Compose UI — Case Detail Insights" and "Stats & visual data prep" rows rewritten for the three-band behaviour. No `DEV_PLAYBOOK.md` / `CLAUDE.md` / `README` change.

**Tests:** `InsightsTabStateTest` — the below-minimum test became a zero-events `NothingLogged` assertion; added "a single event renders the heatmap plus Rhythm and Gaps but not Frequency or Trend" (90-day span, so the event count — not the span — is what hides Trend), "a second event restores the Frequency card", and "the Trend card appears at exactly two events once the span qualifies" (same 90-day span, pinning the guard's upper boundary from both sides). `CaseDetailInsightsTabTest` (`@UiTest`) — `belowInsightsMinEvents_…` split into `zeroEvents_showsFlatInvitation_notAnEmptyChart` and `singleEvent_showsHeatmapRhythmGapsAndCountNote_butNotFrequencyOrTrend`; `atInsightsMinEvents_…` gains a note-absent assertion; class KDoc reworded. `CenteredEmptyStateTest` — test/preview data moved to `IntenseVoice.insightsNothingLoggedMessage` (longest, tied with Bright). `ShareCardStateTest` — the two `NotEnoughData` fixtures → `NothingLogged`; the "leaves every section null" test renamed; added a single-event `Ready` case pinning that its share card offers Rhythm/Gaps but not Frequency/Trend — a deliberate, accepted consequence of the state now being `Ready` at one event. `VoiceTest` / `InsightsEngineTest` / `StatsEngineTest` unchanged.

**Verified:** `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green. The instrumented changes (`CaseDetailInsightsTabTest`, `CenteredEmptyStateTest`) were **not run** — no emulator/device in this environment; they need a device run before merge.
...
**PROGRESS.md was mid-edit at branch start.** The committed A10 was a small "verdict engine's duration handling is unreviewed" read-through item (branch `feat/verdict-duration-review`); an uncommitted working-copy edit had already expanded it into this L-scoped feature spec (the acceptance criteria this pass followed) and, separately, added item **S14** (Hunch-nudge "Don't ask again"). Both the A10 expansion→removal and the S14 addition ride in this commit — S14 kept in deliberately (it came with the setup and is a small standalone entry), the A10 spec net-removed now that it's done. The spike `CoverageRatePrototypeTest.kt` and the concept mockup `verdict-metric-window-prototype.html` were also pre-existing untracked setup; both are removed from the repo — the spike deleted (superseded by real `VerdictEngineTest` cases, and its own KDoc said "expected to be deleted"), the mockup moved to the session scratchpad, per the A10 spec's own "not committed to the repo" note and the S5 precedent for prototype artifacts.

**Design decisions taken with the user (via AskUserQuestion, before building):**
- **Days-active verdict copy is its own string set** (`verdictHeadlineDaysActive` / `verdictMetaDaysActive`), not a `metric` param bolted onto the existing occurrence functions — each metric is phrased on its own terms (a share of days vs. a count of occurrences).
- Schema shape: `metric` / `observationWindow` / `windowStartDate` (nullable, set only for `CUSTOM`), stored as TEXT via the existing `Converters` pattern, **AutoMigration** 7→8 with `@ColumnInfo(defaultValue = …)` — consistent with the v6→v7 auto-migration precedent.
- Everything else was already **decided** in the (expanded) A10 entry (fixed 90-day rolling window matching `DAYS_PER_MONTH`'s approximation; custom date floored at `createdAt`; confidence tiers unchanged, active-day count substituted for event count; two same-day events = one active day; verdict card stays text-only; flat sheet, no disclosure).

**Found & fixed (checklist walk-through against the real `git diff` of every changed file):**
- **`computeVerdict` now excludes events after `now`** — the old engine did a bare `events.size`, relying on callers never passing a future event (only `toHistoryEntry` did, via its own `occurredAt <= resolvedAt` filter). With the window filter in, `it.occurredAt <= now` was added alongside the span-overlap check so a contrived/imported future event can't inflate a count. Regression-covered by the rolling-window "result changes as now advances" test.
- **`hunchWindowCustomDateDescription` Voice key was declared but unused** — caught walking Dead Code. Wired to the custom-date `OutlinedButton`'s `contentDescription` (its visible text is just the formatted date, no context for a screen reader) rather than dropped.
- **`activeSpanEnd`'s tests moved with it** — extracting it to `domain/` left its four cases stranded in `viewmodel/OngoingEventTest.kt`; moved to a new `domain/ActiveSpanTest.kt`. `OngoingEventTest`'s now-unused `tracksDuration` import removed.
- **`confidenceTierFor` / `hunchProgressFraction` params renamed** `eventCount` → `observationCount` (and local `eventFraction` → `countFraction`), since they now receive an active-day count for a days-active Hunch. Behaviour unchanged.
- **Import validation added** (`BackupValidationResult.kt`) — a Hunch whose `windowStartDate` presence disagrees with its `observationWindow` (set iff `CUSTOM`) is rejected, mirroring the `TriggerEntity` `windowDays`-vs-`kind` check right below it. +3 `BackupValidationResultTest` cases.
- **Date-picker UTC round-trip de-duplicated** (checklist finding, second pass) — `HunchCreationSheet`'s new date picker and `LogDetailSheet`'s existing one both hand-rolled the `Instant → LocalDate → atStartOfDay(UTC)` conversion Material3's `DatePicker` needs. Extracted `toDatePickerUtcMillis` / `datePickerDateAtLocalStartOfDay` beside the existing `applyPickedDate` in `viewmodel/LogDetailViewModel.kt`; both sheets now call them. `LogDetailSheet` loses its now-dead `ZoneOffset` import. +3 JVM tests (`LogDetailViewModelTest`), covering a New-York-zone date and a round-trip.
- **`HunchCreationSheet` sections extracted** (checklist finding, second pass) — the four inline `Column { Text(label); … }` blocks now go through a `LabelledSection(label) { content }` helper, matching `LogDetailSheet`'s `TimeSection`/`EndTimeSection` structure; the metric picker's inline "snap `expectedPer` when the metric flips" logic became pure `periodOptionsFor(metric)` / `coerceExpectedPer(metric, current)` helpers (`internal`, +5 JVM tests in `HunchCreationSheetLogicTest`).
- **`verdictMeta` / `verdictMetaDaysActive` Preliminary-tail de-duplicated** (checklist finding, second pass) — the "a few more weeks…" sentence was written twice per voice. Now a per-voice `verdictPreliminaryTail` `val` + a shared `verdictMetaLine(base, tier)` default method; both functions pass just their base sentence. Intense's confident-tier "borne out across" flourish was dropped for a consistent "over" in both tiers (B2 can restore a flourish if wanted).
- **`hunchProgressLabel` / `hunchProgressLabelDaysActive` collapsed to one function** (checklist finding, second pass) — the days-active variants across the three voices differed only by tacked-on filler ("on record", "so far!") to satisfy the "no identical string" `VoiceTest`, since "active days" has no voice-flavoured synonym the way events/entries/logs does. Replaced with a single `hunchProgressLabel(observedCount, unit, windowDays)` default method plus a per-voice `hunchProgressUnitEvents` ("events"/"entries"/"logs") and a shared `hunchProgressUnitDaysActive` ("active days") — the structure is identical by design, the only per-voice part is the noun. The `CaseDetailScreen` call site picks the unit with one `if`.
- **`hunchCheckInDays` + `ExpectedPer.QUARTER`** (checklist finding, second pass) — a days-active Hunch's check-in interval derives from `period ÷ count` treating expected active days as an event gap; a loose reading but §11 timing is an explicit heuristic and the 3–30-day clamp keeps it sane. Noted in the function KDoc; +1 `CheckInTest` locking two `QUARTER` cases (one clamps, one inside bounds).

**Sections walked, nothing to do (checked against the actual diff):**
- *Duplication — the `if (hunch.metric == DAYS_ACTIVE)` dispatch in `HunchVerdictCard` (headline + meta)* — **considered and declined.** Two short branches, each naming a distinct Voice key — a dispatcher would hide *which* key renders, inconsistent with `verdictHeadline`/`verdictMeta` already being separate explicit calls. (The early-card progress label and `formatRate`/`formatExpectedFrequency` *were* de-duplicated — see above — because their per-metric difference was a single noun/token, not authored copy.)
- *Decoupling* — `computeVerdict` gained a `durationMode: DurationMode` param (needed for span math); `DurationMode` is a `data` enum, no `android.*`, engine stays pure; time still flows in as `now: Long`.
- *Complexity* — `HunchCreationSheet` main composable is now ~90 lines of flat `LabelledSection` calls; the date dialog is a private helper reusing `voice.logSheetPickerConfirm`/`Cancel`.
- *`BACKUP_SCHEMA_VERSION` bump* — **considered and declined.** The three fields have Kotlin defaults, so Moshi fills them for a pre-A10 backup that omits the keys (+1 `BackupSerializerTest` case). `BackupSerializer`'s own doc says the version bump lands only when an old payload *can't* be read by defaults alone; v6→v7 set the same precedent (version stayed 1). The valuable third piece of SPEC §17's "three changes" note — import validation — was added.
- *Hardcoded values* — `DAYS_PER_QUARTER = 90.0` is a named `domain` constant beside `DAYS_PER_WEEK`/`DAYS_PER_MONTH`. `EXPECTED_COUNT_RANGE` unchanged (1..99 for both metrics, following the "99 times per day" no-clamp precedent).
- *Naming* — `VerdictMetric` / `ObservationWindow` / `ActiveSpan.kt` sit in `data/` / `domain/`; enum values SCREAMING_SNAKE; the new `Voice` keys follow the `hunchCreating*` / `hunchMetric*` / `hunchWindow*` / `verdict*` families, added to all three voices this commit.
- *Accessibility* — the one new icon-free tappable got a `contentDescription`; the pickers are `SegmentedChoiceRow` (already `Role.RadioButton`).
- *`activeSpanEnd` placement* — a new `ActiveSpan.kt` rather than folding into `CalendarGrid.kt` beside `datesCovered`/`spansMultipleDays` (its KDoc siblings). Either works; the new file keeps `CalendarGrid.kt` about the month grid. Not worth churning.
- *`hunchProgressLabelDaysActive` per-voice thinness / new days-active verdict copy* — the collapse above removes the progress-label case entirely; the 15-branch `verdictHeadlineDaysActive` / `verdictMetaDaysActive` are genuine per-cell authored copy and stay in scope for the B2 voice audit like every other copy-touching item.
- *Deprecated APIs* — none new; `DatePicker`/`rememberDatePickerState` already opted into in `LogDetailSheet`.
- *Repo hygiene* — `git status` clean; new committed files are source + generated `schemas/8.json` (like 1–7). No secrets, no local paths. The spike and concept mockup are out of the tree (see Scope).

**Deferred:** nothing — every finding was fixed or explicitly declined in this pass.

**Docs updated:** `HODITH_SPEC.md` §7 (Hunch-creation pickers; frozen-at-`resolvedAt` note) and §8 (the full window/metric/filter model; text-only card rule). `PROGRESS.md` — A10 removed (resolved); Story-A line and "Fully isolated" bullet updated; S14 addition (pre-existing) rides along; new item **S15** added (audit `docs/mockups/` — this pass surfaced that the dir holds six prototype HTMLs, three orphaned and three still load-bearing for open theme/share items, and the removal is its own gated cleanup). `CLEANUP_CHECKLIST.md` — new Dead Code & Hygiene item: a throwaway prototype (spike / mockup / scratch Preview) must be cleared out once the real work lands, and `git status` checked for untracked ones (the gap that let this pass's own mockup nearly slip through). `TESTING.md` — Verdict engine, Export/import, Room migrations, Compose UI rows extended. No `DEV_PLAYBOOK.md` change.

**Tests:** unit (`./gradlew test`, 610 green) — `VerdictEngineTest` +13, `HunchTabStateTest` +1 (rolling-window history freeze), `CaseDetailFormattingTest` +2, `BackupSerializerTest` +1, `BackupValidationResultTest` +3, `CheckInTest` +1, `LogDetailViewModelTest` +3 (picker helpers), new `domain/ActiveSpanTest` (4, moved), new `ui/casedetail/HunchCreationSheetLogicTest` (5). Instrumented (emulator, scoped) — `CaseDetailScreenTest` +5, `DatabaseFreshInstallTest` +1 (`migrationFrom7To8`); 60 across `CaseDetailScreenTest` / `CaseDetailInsightsTabTest` / `DatabaseFreshInstallTest`, all green.

**Verified:** `ktlintCheck → lintDebug → testDebugUnitTest (610) → assembleDebug` sequential, all green; scoped `connectedDebugAndroidTest` (60) green on `emulator-5554`.
...
## fix/widget-plain-fidelity

**Scope:** PROGRESS.md item S2 — both widgets hand-rolled every `TextStyle`/`GlanceModifier` value as a local literal in `widget/WidgetCommon.kt`/`ListWidget.kt`/`SingleCaseWidget.kt`. Colours were already sourced correctly from `Color.kt`'s Plain-light vals, but type sizes, weights, spacing, and corner radii were never checked against `ui/theme/Type.kt`/`Shape.kt` or Home's actual Plain rendering. The user also asked for a mechanism to keep the widget in sync if Plain's theme changes later, beyond a one-time fix.

**Design decisions taken with the user before building:**
- The red "+" glyph report that used to share this PROGRESS.md item was split into its own item (S12): source already shows `WidgetPalette.accent` (a teal) is the only color ever applied to the "+" glyph in every state, with no in-widget Stop control at all — what's left is strictly an on-device photograph pass, outside what this session does.
- New token-size assertions extend the existing instrumented tests (`WidgetActionsFlowTest`, `WidgetChromeNavigationTest`) rather than creating `ListWidgetTest`/`SingleCaseWidgetTest` — PROGRESS.md's references to those files were stale; they never existed.
- "Follow Plain if it changes later" is built as a drift-detection unit test (`WidgetTokenFidelityTest`), not live token consumption — Glance can't consume M3's `Typography`/`Shapes` directly, so a test that reads the real resolved Plain values and fails on mismatch is the mechanism, not a shared import.

**Found & fixed:**
- `WidgetCornerRadius` was `16.dp` — literally 2× the `8.dp` Home's Plain plank `Card` actually renders at (`HomeScreen.kt`'s `PlainPlankHomeCaseListItem` takes no explicit `shape`, so it resolves to M3's default `Card` shape, `Shape.kt`'s `medium`). Changed to `8.dp`.
- `ListWidget.kt`'s row icon was `20.sp`, `SingleCaseWidget.kt`'s was `24.sp` — same glyph role, silently drifted apart. Unified on `WidgetIconGlyphSize = 24.sp` (matches Home's `headlineSmall`).
- The "+" glyph was `20.sp` in `ListWidget.kt`, `18.sp` in `SingleCaseWidget.kt`. Unified on `WidgetPlusGlyphSize = 20.sp`.
- `SingleCaseWidget.kt`'s "Case not found" text (`12.sp`) and `ListWidget.kt`'s empty-state text (`13.sp`) were the same widget-only informational-placeholder role, inconsistent the same way. Unified on `WidgetInfoMessageSize = 14.sp`.
- Every remaining inline `TextStyle`/`GlanceModifier` literal in both widgets replaced with a named `WidgetCommon.kt` token (17 new tokens total) — case name, subtitle, Ongoing-pill text/padding/shape, icon-to-text spacing, and widget-only layout spacing that has no Home analogue (documented as such rather than blindly copied from a phone-width screen).
- Added `WidgetTokenFidelityTest.kt` (JVM unit test): reads Plain's actual resolved `Type.kt`/`Shape.kt` values via the existing public `hodithTypography`/`hodithShapes` functions and asserts the widget tokens match — ran green on first pass, confirming the derived values are correct, and will fail on a future Plain theme change that isn't reflected in `WidgetCommon.kt`.
- Extended `WidgetActionsFlowTest.kt`/`WidgetChromeNavigationTest.kt` with rendered-`TextView.textSize` assertions for the newly-centralised tokens, via two new helpers (`findTextViewWithText`, `spToPx`) added to `WidgetRenderTestFixtures.kt`, following that file's existing real-`AppWidgetHost`-render pattern rather than adding a Glance testing dependency.
- On-device: an initial `WidgetActionsFlowTest` assertion for `WidgetCaseNameSize` (`singleCaseWidget_caseName_rendersAtWidgetCaseNameSize`) failed real-device execution and was removed — it wrongly assumed the Single-case widget renders `row.name`; reading `SingleCaseWidget.kt` again shows it only ever shows the icon and subtitle. The List widget's `CaseRow` does render the name, but its content lives inside a `LazyColumn` this test harness can't populate (documented limitation). `WidgetTokenFidelityTest`'s JVM assertion remains the only fidelity coverage for that token.
- Local widget-test bind-permission docs were wrong: `WidgetActionsFlowTest`/`WidgetChromeNavigationTest`/`ListWidgetConfigureFlowTest`/`SingleCaseWidgetConfigureFlowTest`'s class doc comments and DEV_PLAYBOOK §5 said to `grantbind` the app package (`com.secondmonday.hodith`); on a local Windows Google Play/GMS-enabled emulator image, that silently grants nothing (`dumpsys package` showed the app and its `.test` package hold distinct UIDs there) and every widget instrumented test failed with `bindAppWidgetIdIfAllowed failed` despite `grantbind` reporting success — granting `com.secondmonday.hodith.test` instead (confirmed via a direct `adb shell am instrument` run) fixed it. Checked whether this meant CI was silently broken too (`gh run view` across several recent merged PRs) — it wasn't: CI's AOSP `default` image passes 88/88 repository-shard tests granting the app package, so the correct package is emulator-image-dependent, not a universal fact. DEV_PLAYBOOK §5 and the four test class doc comments now note both cases; `.github/workflows/instrumented-tests.yml` was left untouched since it's empirically working.

**Sections walked, nothing to do (checked against the actual diff/repo state, not from memory):**
- *Duplication* — this pass removes duplication rather than adding it (three literal pairs unified into one token each, listed above). No new composables added; no new `Voice` strings added (all `Text()` calls still reference existing `PlainVoice.*` keys, untouched).
- *Decoupling* — no ViewModel/Repository/domain code touched; `produceState` blocks in both widgets untouched.
- *Complexity & Pattern Health* — no new nesting; the new `findTextViewWithText`/`spToPx`/`waitForTextViewWithText` helpers are each used by 3+ call sites across the two test files, and `WidgetTokenFidelityTest`'s `cornerRadiusDp` helper is used twice in one test — all earn their extraction.
- *Dead code* — `ktlintCheck` (enforces no-unused-imports) passed clean on all touched files, including the `sp`/`dp` import churn in `ListWidget.kt`/`SingleCaseWidget.kt`/`WidgetCommon.kt` from centralising literals into tokens.
- *Repo hygiene* — `git status` after all edits shows exactly the six widget/test source files plus `WidgetTokenFidelityTest.kt` (new) and `DEV_PLAYBOOK.md`/`HODITH_SPEC.md`/`PROGRESS.md`/`CLEANUP_LOG.md` — nothing stray or secret-shaped.
- *Naming Consistency* — `WidgetTokenFidelityTest.kt` sits in `app/src/test/kotlin/.../widget/`, matching the existing `FakeWidgetRefresher.kt`'s package placement and the repo's `*Test.kt` convention.
- *Hardcoded Values* — the new tokens are the fix for this category, not a new instance of it; they're UI/widget-layout constants (matching `MinTapTarget`'s existing placement in `WidgetCommon.kt`), not the domain-layer product constants (verdict thresholds, confidence tiers) CLAUDE.md's rule targets.
- *Accessibility* — `MinTapTarget` (48dp) untouched, still applied at the same call sites; none of the new tokens affect tap-target sizing.
- *Deprecated APIs* — `compileDebugKotlin`/`compileDebugAndroidTestKotlin` produced no new deprecation warnings for any touched file (the one pre-existing Moshi/Kapt warning in the build log is unrelated, from Hilt codegen).
- *Spec Review* — `HODITH_SPEC.md` §15 gained the one-sentence pointer to DEV_PLAYBOOK §4 the item's acceptance criteria asked for; no other section needed updating.
- *Tests — TESTING.md* — checked whether the new `WidgetTokenFidelityTest`/token-size assertions need a TESTING.md row: `HodithThemeTest`, an existing near-identical "guards against copy-paste drift across theme definitions" JVM test, isn't mentioned anywhere in TESTING.md either — its "Planned unit coverage" table tracks functional/business-logic areas, not internal design-token consistency guards. Consistent with that established precedent, no TESTING.md change needed.
- *Tests — MANUAL_TEST_PLAN.md* — no new flow crosses a system-process boundary; not touched.
- *Tests — @UiTest/@Smoke* — no new instrumented test *class* added (only new test methods on the existing `WidgetActionsFlowTest`/`WidgetChromeNavigationTest`), and neither of those classes is `@UiTest`-tagged already (confirmed via grep and TESTING.md's own note that `WidgetActionsFlowTest` is deliberately untagged) — nothing to tag.

**Deferred (explicit sign-off given during plan approval, before this pass):**
- Row layout/header/empty-state visual reconciliation against Home's Plain rendering (Compose Preview eyeball pass) and the `WidgetPillCornerRadius = 999.dp` stadium-shape sanity check — visual review this session doesn't do; PROGRESS.md's S2 keeps these as open checklist items.
- The red "+" glyph's on-device root-cause repro — split into PROGRESS.md's new S12; source-level investigation is exhausted (no color path but `accent`, no in-widget Stop control), the remainder is strictly hands-on-device work.

**Docs updated:** `DEV_PLAYBOOK.md` §4 (full widget token-mapping table + the two Glance structural gaps) and §5 (the grantbind package finding above); `HODITH_SPEC.md` §15 (one sentence); `PROGRESS.md` (S2 narrowed to this fidelity work with its done items checked off; new S12 for the red-"+" investigation; the on-device QA batching note updated to include S12); the four widget instrumented test classes' doc comments (grantbind package finding).
...
**Design decisions taken with the user before/while building:**
- **Remove the stale prompt outright, not keep it on Case Detail only.** The Ongoing pill + ticking elapsed ("2d 4h" past a day) already signals staleness on both screens, and on Home the prompt's "Edit end time" action was misleading (it just navigated to Case Detail). Consistent with §4 — HODITH observes, it doesn't prod.
- **Drop the column with a Room `@DeleteColumn` auto-migration (schema v6 → v7), not a hand-written `Migration`.** SQLite `DROP COLUMN` is API 34+ and minSdk is 31, so a manual migration would mean the 12-step table-recreate by hand; the auto-migration generates it. `SchemaMigrationCoverageTest` learned to count auto-migrations (`HodithDatabase.AUTO_MIGRATION_COUNT`) since they aren't in the `MIGRATIONS` array.
- **App-managed group summary, not Android's auto-generated one.** With minSdk 31 a bare `setGroup` gets an auto-summary, but `GROUP_ALERT_SUMMARY` with no summary present risks a silent lone notification, and the auto-summary's copy isn't ours. The app-managed summary (`SystemNotifier.refreshGroupSummary`, recomputed from `NotificationManager.activeNotifications` after every post/cancel) is the safe, voiced option. User confirmed keeping it over the lighter auto-summary path.
- Commit messages: subject-line only, no `Co-Authored-By` trailer (repo convention; confirmed with the user against this session's attribution instruction).

**Found & fixed (checklist walk-through against the real diff):**
- **Answering "All quiet" on one of two grouped check-ins wiped the other one** — caught by a new instrumented test (`NotificationActionReceiverTest`) on an API 36 emulator. `refreshGroupSummary` cancelled the group summary as soon as the child count dropped below 2, and on Android **cancelling a `setGroupSummary(true)` notification cascades to the group's remaining children**. Fix: `syncGroupSummary` only cancels the summary at *zero* children (nothing to cascade); at one child it leaves the summary in place (Android suppresses a lone-child summary and shows the child standalone).
- **The summary count raced itself in the batch check-in pass.** `evaluateCheckIns` fires several `notifyCheckInDue` / `cancelCheckIn` in a row, and `refreshGroupSummary` read `NotificationManager.activeNotifications` straight after each — before the `notify`/`cancel` had landed — so the last write could leave an orphaned "2 cases" summary over zero children. `syncGroupSummary(voice, addedId, removedId)` now takes the just-changed id as a hint and adjusts the live read, so it never counts a notification it just cancelled or misses one it just posted.
- **The POST_NOTIFICATIONS permission check was duplicated.** Lint's `MissingPermission` doesn't follow a `Boolean`-returning helper, so the inline `ActivityCompat.checkSelfPermission(...) != GRANTED` guard ended up copied into both `post()` and `refreshGroupSummary()`. Extracted a single `private fun notify(id, notification)` that does the check immediately before `NotificationManagerCompat.notify` — lint recognises that shape, and it's the one posting path now.
- **`HomeCaseRowBody` / `BrightHomeCaseListItem` each wrapped their content in a one-child `Column`** once the trailing stale banner left. Collapsed both — the row body is just its `Row` now, and Bright's item padding moved onto `GlowCard`'s `modifier`.
- **`TESTING.md` carried five stale claims:** the notification-evaluation unit row's "2+ due check-ins collapse into one summary"; the ViewModels row's "reopen … rebases `staleNudgeDismissedAt`"; the Room-migrations row (no mention of v7); the Compose-UI row's "reached from … the stale-ongoing prompt" + "consolidated multi-stale banner"; and manual-check item 10's "arrive as one summary notification". All rewritten to the grouped-notifications / no-stale-prompt reality.
- **Spec §6 used negative framing** ("a long-running open event isn't nagged") — reworded to state what it does (carries the same pill + elapsed signal as any event) per the positive-framing house rule.

**Sections walked, nothing to do:** *Duplication* — the row-`Column` collapse above was the only extraction opportunity; the new helpers (`openAppPendingIntent` 2 callers, `activeGroupChildren` / `notify` 1-2 callers) each replace a gnarly inline expression and earn their keep. No inline user-visible strings — the summary title is a new `Voice` key (`notificationsGroupSummaryTitle`, renamed from `checkInsSummaryNotificationTitle` since it now spans triggers too), its `InboxStyle` lines reuse the children's own already-voiced titles. *Decoupling* — `Notifier` lives in `notification/`, not `domain/`, so its new `android.app.Notification`/`NotificationManager` imports are fine; no `Clock` needed (nothing time-dependent); nothing in `domain/` touched. *Complexity* — `HomeScreen`/`CaseDetailScreen` both got shorter; `syncGroupSummary` is ~30 lines with a `when` over the child count. *Dead code* — `CHECK_IN_SUMMARY_NOTIFICATION_ID` (2_000_000) gone with `notifyCheckInsSummary`; `isStaleOngoing`, `STALE_ONGOING_THRESHOLD_MILLIS`, `dismissStalePrompt` ×2, the `LogDetailViewModel` reopen-rebase, 4 stale `Voice` keys ×3 voices, and every stale-prompt test removed; `ktlintCheck` `no-unused-imports` green. *Repo hygiene* — `git status` clean; the new `schemas/…/7.json` belongs in the repo like the other `N.json` (Room's `room.schemaLocation`); nothing secret-shaped, no local paths. *Naming* — `notificationsGroupSummaryTitle` matches the other `notification*` keys; `DropStaleNudgeColumn` / `SCHEMA_FREEZE_POINT` / `AUTO_MIGRATION_COUNT` read clearly. *Hardcoded values* — `GROUP_SUMMARY_NOTIFICATION_ID` / `GROUP_SUMMARY_MAX_LINES` / `NOTIFICATION_GROUP_KEY` are named constants. *Accessibility* — no new buttons; the summary notification carries a small icon + content title. *Deprecated APIs* — no new warnings. *Spec Review* — §6, §11, §14 updated (see Docs); §17 Future Work has no item this implements.

**Considered and declined (with the user):**
- **Swapping the app-managed group summary for Android's auto-generated one.** A bare `setGroup` on minSdk 31 gets an auto-summary, which would drop the `syncGroupSummary` bookkeeping — but `GROUP_ALERT_SUMMARY` with no summary present can leave a lone notification silent, and the auto-summary's wording isn't ours (no Voice). The user's call: keep the app-managed summary. Not a deferred task, a closed decision. (The cascade-cancel and count-race bugs above are the cost of that choice, now handled.)

**Deferred:** nothing.

**Docs updated:** `HODITH_SPEC.md` §6 (stale-prompt sentences removed; "Back to ongoing" clause trimmed of the stale-clock rebase; positive-framed replacement), §11 (Anti-spam bullet rewritten around the notification group + app-managed summary + silent re-post + not-due withdrawal), §14 (Edit-event row no longer "reached from … the stale-ongoing prompt"). `TESTING.md` — five spots above. `MANUAL_TEST_PLAN.md` — the two check-in notification items (re-fire, summary collapsing) collapsed into one, trimmed to just what the instrumented tests can't reach: the summary's tap target and a one-time on-device check that the OS honours `GROUP_ALERT_SUMMARY` / `setOnlyAlertOnce`; the behavioural assertions are now automated and cross-referenced instead of re-listed as manual steps. Section preamble scope line updated to match. `PROGRESS.md` — S9 section removed; B2's "copy-touching items still open" line updated to note this branch's Voice changes for the audit. `CLEANUP_CHECKLIST.md` — new rule: a finding is only "deferred" with human sign-off. KDoc on `Notifier` / `syncGroupSummary` / `cancelCheckIn` / `DropStaleNudgeColumn` / `NotificationEvaluator.evaluateCheckIns`.

**Tests:**
- Unit (JVM, all green — 574/0): `NotificationEvaluatorTest` — "collapses 2+ into a summary" → "posts an actionable check-in for every due Case" and "withdraws the check-in for an active Case that is not due"; new "evaluateCase withdraws the check-in once the Case is no longer due"; the not-auto-rearm test now also asserts no spurious `cancelCheckIn`. `SchemaMigrationCoverageTest` reworked to `MIGRATIONS.size + AUTO_MIGRATION_COUNT` against a `SCHEMA_FREEZE_POINT`. Stale-prompt tests deleted from `OngoingEventTest` / `CaseDetailViewModelTest` / `HomeViewModelTest`; `LogDetailViewModelTest`'s three `staleNudgeDismissedAt` tests → two that keep only the `endedAt` reopen/edit assertions.
- Instrumented (**run on an API 36 emulator — 63/63 across the touched classes**): `DatabaseFreshInstallTest` gains a v6→v7 `MigrationTestHelper` test (column gone, rows survive). `NotifierContentTest` — the summary test → `twoDueCheckIns_shareTheGroup_andGetAnAlertOnceSummary` (group key, `GROUP_ALERT_SUMMARY`, `FLAG_GROUP_SUMMARY`, `FLAG_ONLY_ALERT_ONCE`), `cancelCheckIn_droppingToOneChild_keepsTheSurvivingCheckIn`, `cancelCheckIn_forEveryChild_removesTheSummary`; new `notifyCheckInDue_setsOnlyAlertOnce…`. `NotificationActionReceiverTest` gains `actionAllQuiet_withOneCheckInRemaining_leavesTheSurvivingCheckInShowing` — posts two real check-ins via the injected `Notifier`, broadcasts All quiet for one with its real `checkInNotificationId` (made `internal` for this), asserts the sibling check-in survives; `insertedCaseId` → a tracked list, `cancelAll()` added to teardown. `HomeScreenTest` / `CaseDetailScreenTest` / `CaseDetailInsightsTabTest` — stale-banner tests deleted, harness `onDismissStalePrompt` params dropped.

**Verified:** `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green; full unit suite 574/0. Instrumented on `emulator-5554` (`Pixel_8_API36`, API 36): `NotifierContentTest` 6/6, `NotificationActionReceiverTest` 4/4, `DatabaseFreshInstallTest` 2/2, `HomeScreenTest` 10/10, `CaseDetailScreenTest` 20/20, `CaseDetailInsightsTabTest` 21/21 — 63/63. The first emulator run flagged the group-summary cascade bug (now fixed); a follow-up run flagged the count race (now fixed); the run above is clean.

---

## feat/event-edit-screen

**Scope:** PROGRESS.md Standalone item S8 — editing an existing event reused `LogDetailSheet` (a `ModalBottomSheet` with no visible dismiss control), while editing a Case is a full screen with a `TopAppBar` back arrow, so the two edit flows read differently. User's call: make them match. Editing an event is now a full-screen destination (`LogDetailScreen`) mirroring `CaseEditScreen`; logging a *new* event (quick-log, retro-log FAB, widget trampoline) stays the sheet, since that flow is meant to be ≤5 seconds (spec §6).

**Design decisions taken with the user before/while building:**
- **Direction:** screen for edit, sheet for new — not "add a close X to the sheet" (smaller, but leaves the structural mismatch) and not "everything becomes a screen" (fights the quick-log intent, turns the transparent widget trampoline into a full activity, biggest spec rewrite).
- **Compose-Preview spike first.** Extracted the shared `LogDetailForm`, built `LogDetailScreen` + previews (Plain light/dark, Intense, Bright), stopped for the user to eyeball the chrome against `CaseEditScreen` before wiring nav/ViewModel. Approved, then continued.
- **The edit screen gets a real `@HiltViewModel`** (`LogDetailScreenViewModel`, keyed by `caseId` + `eventId` nav args, mirroring `CaseEditViewModel`). This isn't a reversal of `LogDraft`'s "the sheet has no ViewModel" note — that reasoning was *"opened from more than one nav destination rather than owning its own back-stack entry"*; the edit screen *does* own one. The sheet still has none. All the actual save/delete logic is the existing pure `planSaveEvent` / `tagDiff` / `draftFrom`.

**Found & fixed (checklist walk-through against the real diff):**
- **`LogDetailSheet`'s `isEditing` and `onDelete` params were dead** once editing moved out — every remaining caller (Home, widget trampoline, Case Detail FAB) passes `isEditing = false` and no `onDelete`. Removed both, inlined the new-event title as a plain `Text` (the whole `SheetHeader` private composable + its delete `IconButton` + the sheet's `showDeleteConfirm` state and `ConfirmDialog` went with them). `Icons.Filled.Delete` import dropped. Delete now lives only in `LogDetailScreen`'s `TopAppBar`, via the shared `DeleteEventConfirmDialog` extracted from what the sheet used to inline.
- **`CaseDetailViewModel.saveEvent(draft, existingEvent, originalTags)` and `deleteEvent`** were only ever reached from the sheet's edit path. Renamed to `saveNewEvent(draft)` (insert-only — drops the `plan.isUpdate` branch and the `tagDiff.toRemove` loop, both unreachable for a fresh event); `deleteEvent` removed (Home has its own `deleteEventById`, unaffected). `planSaveEvent` call switched to all-named args since two are now literal `null` / `emptyList()`.
- **New `.kt` files were written LF; the repo is CRLF** (no `.gitattributes`, `core.autocrlf=true`). `ktlintFormat` also flipped the edited `CaseDetailViewModel.kt` to LF. Converted all five back to CRLF (`perl -i -pe 's/\r?\n$/\r\n/'`), matching the `feat/log-sort-by-end` / `test/duration-edge-coverage` passes; `git diff` shows no EOL churn and `ktlintCheck` stays green.

**Sections walked, nothing to do:** *Duplication* — the sheet and the screen now share `LogDetailForm` (the whole field set + pickers) and `DeleteEventConfirmDialog` rather than diverging; the screen's `Scaffold` `containerColor` branch is copied from `CaseEditScreen` verbatim (a 4-line Plain-surface rule, not worth a third helper for two call sites). No inline user-visible strings — the title reuses `logSheetNewEventTitle` / `logSheetEditEventTitle`, the back arrow `backButtonDescription`, the delete action `deleteEventConfirmAction`; **zero new `Voice` keys**. *Decoupling* — `LogDetailScreenViewModel` holds no UI types; `LogDetailScreen` takes a `uiState` + three lambdas, not the ViewModel; no `System.currentTimeMillis()` (injected `Clock`); nothing in `domain/` touched. *Complexity* — `LogDetailForm` ~110 lines, `LogDetailScreen` ~90 (+ previews); `LaunchedEffect(uiState.isFinished)` in `LogDetailRoute` mirrors `CaseEditRoute`'s `isSaved` effect exactly. `remember` (not `rememberSaveable`) for the draft matches what the sheet already did and what `CaseEditScreen` does for its dialog state — see Deferred. *Dead code* — removed `EditRequest`, the `draftFrom` / `TagEntity` imports in `CaseDetailScreen`, the `first` import in `CaseDetailViewModelTest`; `ktlintCheck` `no-unused-imports` green; no leftover TODOs. *Repo hygiene* — `git status` shows only the 16 expected files (4 new `.kt`, 8 edited `.kt`, 3 docs, 1 new dir); nothing secret-shaped, no local paths, no build output. *Naming* — `LogDetailScreen.kt` / `LogDetailScreenViewModel.kt` follow `*Screen.kt` / `*ViewModel.kt`, sit in `ui/logsheet/` beside `LogDetailSheet.kt` and `viewmodel/` beside the others; `LogDetailRoute` lives in the screen file like `CaseEditRoute`. *Hardcoded values* — `PaddingValues(16.dp)` / the sheet's `bottom = 24.dp` are layout constants consistent with the file, not product constants. *Accessibility* — the back arrow (`backButtonDescription`) and delete `IconButton` (`deleteEventConfirmAction`) both carry non-empty `contentDescription`s; M3 `IconButton` meets 48 dp. *Deprecated APIs* — no new warnings (only the pre-existing `DataStoreSettingsRepository` annotation-target one). *Spec Review* — §6 and §17 updated (see Docs); intentional divergence, no bug.

**Deferred:**
- **`LogDetailForm` uses `remember`, not `rememberSaveable`, for the working draft** — a process-death mid-edit loses unsaved changes. This matches the sheet's prior behaviour and `CaseEditScreen`'s own dialog state, and `LogDraft` / `TagEntity` aren't `Parcelable`, so a proper fix is a `SavedStateHandle`-backed draft in the ViewModel — a bigger change than S8's scope. Noted here; pick up if it starts to matter.
- **Bright/Intense rendering of `LogDetailScreen`** — `LogDetailScreenTest` is Plain-only like the rest of the suite; the per-theme `containerColor` branch is exercised by the four Compose Previews and logic review, and stays a manual check like other Bright/Intense UI.

**Docs updated:** `HODITH_SPEC.md` §6 — the Detail-sheet bullet now scopes the sheet to *new* events and states editing is a full screen; "Back to ongoing" and "every event is editable" reworded from "sheet" to "screen". §17 screens table — "Log detail sheet" scoped to new events, new **Edit event** row. `TESTING.md` — "ViewModels" unit row gains `LogDetailScreenViewModel`'s load/save/delete/missing-event contract; "Compose UI" row gains the edit screen's chrome + the `onEditEvent` nav-callback assertions, and the "Back to ongoing" round-trip is re-attributed to it; the retro-log deferral drops "delete-from-sheet". `PROGRESS.md` — S8 section removed (resolved); the Log-tab cluster line now reads S4-only, noting S7 and S8 already landed. KDoc on `LogDetailSheet` / `LogDetailForm` / `LogDetailScreen` / `LogDetailScreenViewModel` / `CaseDetailViewModel.saveNewEvent`.

**Tests:**
- New `LogDetailScreenViewModelTest` (4, JVM) — loads an event's fields + its Case's config into the draft; `save` updates the event and diffs its tags; `delete` removes it; a missing event id finishes straight back with a null draft.
- New `LogDetailScreenTest` (4, instrumented, `@UiTest`, one `@Smoke`) — back arrow invokes `onBack`; delete action opens the confirm and invokes `onDelete`; delete action hidden when `onDelete` is null; a `START_STOP` "Back to ongoing" → save round-trips a null `endedAt` through the shared `LogDetailForm`.
- `CaseDetailScreenTest` — the two edit-mode tests reworked: `editingStoppedEvent_backToOngoing…` (which drove the sheet) → `eventRow_click_invokesOnEditEventForThatEvent` (asserts the nav callback with the right `caseId`/`eventId`); `staleOngoingBanner_editEndTime_opensSheetInEditModeForThatEvent` → `…invokesOnEditEventForThatEvent`. The new-event sheet tests keep working (helper's `onSaveEvent` lambda arity dropped 3 → 1).
- `CaseDetailViewModelTest` — `deleteEvent` test removed, `saveEvent updates an existing event…` removed (both now `LogDetailScreenViewModelTest`'s job); `saveEvent inserts…` → `saveNewEvent inserts an event with its tags`.
- `CaseDetailInsightsTabTest` — `CaseDetailScreen` call updated for the signature change (no behaviour asserted here).

**Verified:** `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green; full unit suite 0 failures (incl. new `LogDetailScreenViewModelTest` 4/4, `CaseDetailViewModelTest` green after the moves). Instrumented on `emulator-5554` (`Pixel_8_API36`): `LogDetailScreenTest` 4/4, `CaseDetailScreenTest` 23/23 (incl. the two reworked nav-callback tests), `CaseDetailInsightsTabTest` 21/21, `HomeScreenTest` 3/3, `WidgetLogTrampolineActivityTest` 10/10 — 61/61. The emulator's system process crashed partway through the first run (`Can't find service: activity` / `package`), aborting during an unrelated `HomeScreenTest`; re-run of the two unfinished classes after it recovered was clean.

---

## feat/log-sort-by-end

**Scope:** PROGRESS.md Standalone item S7 — the Case Detail Log tab was fixed to `ORDER BY occurredAt DESC` (start), with no way to view a duration-tracking Case's events by when they *ended*. Added an inline **Started / Ended** toggle (shown only when `durationMode ≠ NONE` and the log is non-empty), backed by a pure `sortEventsForLog` helper. Same branch: a full PROGRESS.md restructure the user asked for — every shipped item stripped, Story A dissolved, standalone sequencing reviewed.

**Design decisions taken with the user before/while building:**
- Prototyped the control three ways, then two, as a published artifact before writing Compose. Landed on **inline label + `SegmentedChoiceRow`, no info dot** — "Started / Ended" is self-explanatory, and the app has no dropdown/menu/filter pattern anywhere, so a segmented toggle is the only precedent (Insights frequency granularity, Case Edit, Settings).
- **Kotlin-side sort, not a new `EventDao` query.** The sort is transient view state (spec §6: not persisted, no schema field) over an already-loaded list of one Case's events; a DAO query would mean a second `Flow` and flow-switching on every toggle. `EventDao.observeEventsWithTagsForCase` and its newest-first DAO/Fake tests are untouched — it's shared with `CaseEditViewModel` / `ShareViewModel`.
- **Voice keys as interface `get()` defaults**, not abstract ×3 — matching the exact precedent (`insightsFrequencyGranularityDay/Week/Month`) and B2's own carve-out for "field labels and the like, reviewed once, not per voice". A `get()` default is still the Voice layer and still supplies all three voices in one commit; forcing per-voice variance on a one-word segment label is what the frequency toggle already declined to do. `VoiceTest`'s reflection walk covers the new keys by construction (non-blank; `get()` defaults excluded from the "identical across voices" check by design).

**Found & fixed (checklist walk-through against the real diff):**
- **`multipleOngoingEvents_rowStopButton_stopsThatEvent` was passing for the wrong reason** (checklist "Tests" → "existing tests pass for the wrong reason"). It handed `CaseDetailScreen` two ongoing events `[occ 0, occ 1000]` and tapped `onFirst()` Stop expecting the `occ 0` one — relying on the list rendering in unsorted pass-through order, which the real DAO (`ORDER BY occurredAt DESC`) never produces. `sortEventsForLog(BY_START)` now orders the rendered list newest-start-first like the DAO, so the test's `first` was reworked to carry the later `occurredAt` (2000 vs 1000); its intent (row-targeted Stop stops that row's event) is unchanged and now runs against a realistic order.
- **`SingleChoiceSegmentedButtonRow` / the Bright pill track hard-coded `Modifier.fillMaxWidth().padding(top = 8.dp)`**, so `SegmentedChoiceRow` couldn't sit inline. Rather than fork the control or hand-roll a second segmented toggle (duplicating the per-theme selected-segment colour logic), added two params — `modifier` (default preserves the old value byte-for-byte) and `stretchToFill` (Bright's `weight(1f)` → content-size + horizontal padding when false). All 4 existing call sites use the defaults; verified unchanged by re-running `CaseDetailInsightsTabTest` + `CaseEditScreenTest` + `SettingsScreenTest` (60/60 green).

**Sections walked, nothing to do:** *Duplication* — no copied composables; the sort `Row` reuses the summary line's `padding(horizontal = 16.dp)` and `labelLarge` (as `LabelWithInfo` does); the one new user-visible string trio goes through `Voice`. *Decoupling* — `sortEventsForLog` is pure (`data.*` only, no `Clock`, no `android.*`, not in `domain/`); the composable calls it exactly where it already calls `insightsTabState` / `ongoingEventsIn`; `LogTabContent` gets `sortOrder` + a lambda, not a ViewModel. *Complexity* — `LogTabContent` ~86 → ~110 lines (< 150); `logSortOrder` uses `remember` to match `frequencyGranularityOverride` two lines above (a transient view pref, resetting on config change is fine); `.then(if (stretchToFill) …)` mirrors the file's existing `.then(if (isSelected) …)` idiom. *Dead code* — `ktlintCheck` `no-unused-imports` green; no commented-out code, TODOs, or self-updating doc tallies (TESTING.md edits are prose). *Repo hygiene* — `git status` shows only the expected files (2 new `.kt`, 4 edited `.kt`, 3 docs); nothing secret-shaped, no local paths, no build output. *Naming* — `LogSort.kt` / `LogSortTest.kt` sit in `viewmodel/` beside `OngoingEvent.kt` (pure presentation helpers), mirroring each other; `LogSortOrder` / `sortEventsForLog` descriptive, no abbreviations. *Hardcoded values* — `4.dp` / `16.dp` are layout constants consistent with the surrounding file, not product constants. *Accessibility* — segments carry text labels (not icon-only); M3 `SegmentedButton` meets the 48 dp target; the Bright inline path keeps the shipped `vertical = 7.dp` and adds `horizontal = 16.dp`, no worse than the existing stretched form. *Deprecated APIs* — no new warnings (only the pre-existing Moshi-kapt one). *Spec Review* — §6 gains one bullet stating the toggle; no §17 Future Work item covered this.

**Deferred:**
- **Bright/Intense rendering of the inline sort toggle** — no instrumented coverage. `CaseDetailScreenTest` is Plain-only (like the rest of the app bar `BigPictureScreenTest`, "the only test that provides `LocalCardDecorationStyle`"); the `stretchToFill = false` Bright branch is exercised by logic review + the existing `SegmentedChoiceRow` Bright Previews, and stays a manual check like other Bright UI.

**Docs updated:** `HODITH_SPEC.md` §6 — one bullet: the list is newest-first by start, a duration-tracking Case also gets the Started/Ended toggle, view-state-only. `TESTING.md` — "ViewModels" unit row gains `sortEventsForLog`'s ordering contract; "Compose UI" row gains the Log-tab toggle's visibility gate + reorder. `PROGRESS.md` — S7 removed (resolved); Story A dissolved (A1–A9 + Satellite stripped, A10 moved to Standalone with the shipped sections gone); "How this file is organised" and B2's "must land last" list refreshed; Standalone intro rewritten with the explicit sequencing review (S9→B2 ordering; S2·S4·S5·S3 on-device batch; S4·S8 Log-tab cluster; S1/S6/A10 isolated). KDoc on `LogSort.kt` and the extended `SegmentedChoiceRow`.

**Tests:**
- New `LogSortTest` (8) — `BY_START` newest-first + id tie-break; `BY_END` floats running `START_STOP` events above finished, orders finished by `endedAt` desc not start, orders several runners newest-start-first, `MANUAL` end-less entry falls back to `occurredAt` (not floated), `NONE` never floats, `endedAt` ties break by id.
- `CaseDetailScreenTest` +3 (instrumented, `@UiTest`) — toggle hidden for a `NONE` Case, hidden for an empty log, shown (with both option labels) for a `MANUAL` Case with events; tapping "Ended" floats a running event above a more-recently-started finished one (row-bounds assertion). `multipleOngoingEvents_rowStopButton_stopsThatEvent` reworked (see Found & fixed).

**Verified:** `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green. `LogSortTest` 8/8, `VoiceTest` 3/3, `CaseDetailViewModelTest` / `CaseDetailFormattingTest` unchanged-green. Instrumented on `emulator-5554` (`Pixel_8_API36`): `CaseDetailScreenTest` 23/23, and `CaseDetailInsightsTabTest` + `CaseEditScreenTest` + `SettingsScreenTest` 60/60 confirming the `SegmentedChoiceRow` change is inert for existing callers. First instrumented run caught the double-`setContent` in one new test (split into three) and the `multipleOngoingEvents` regression (fixed); re-run green.

---

## test/duration-edge-coverage

**Scope:** PROGRESS.md Standalone item S10 — the two DST/edge cases the A1–A8 test-coverage audit (`feat/duration-switch-in-conversion`) deferred: the day-math primitives behind duration having only indirect DST coverage (via `datesCovered` in `CalendarGridTest`), and the Big Picture *week* detail dialog never exercising the "lasted …" / "ongoing since …" row copy that the *day* dialog covers. Pure test additions — no production `.kt` change, and none was warranted (the fixed-millis `computeEndedAt` wall-clock shift is by design, §6/§10 storage-is-millis).

**Found & fixed (checklist walk-through against the real diff):**
- **New file `CalendarMathTest.kt` was written LF; the repo is CRLF** (no `.gitattributes`, `core.autocrlf=true`, every existing `.kt` checks out CRLF). `ktlintFormat` also rewrote the two edited unit-test files (`InsightsEngineTest.kt`, `LogDetailViewModelTest.kt`) LF-only. `ktlintCheck` / `testDebugUnitTest` passed regardless and `git diff` showed no EOL churn (autocrlf normalises the blob to LF on commit either way), but converted all three back to CRLF to match the checked-out tree and clear git's "LF will be replaced by CRLF" warning — matching the `feat/time-format-setting` pass. Re-ran `ktlintCheck → testDebugUnitTest --rerun-tasks` green.

**Sections walked, nothing to do:** *Duplication* — `CalendarMathTest` carries a 5-line `noonMillis(date)` local mirroring `CalendarGridTest`'s `noonOn`; no shared helper fits (`TestTime.kt`'s `millisAt` is pinned to `TEST_ZONE`, the whole point here is an explicit non-default zone), and `CalendarGridTest`'s `utcMillis` is UTC-only and file-private. The week-dialog tests reuse the existing `millisAt` / `weekStart` / `case` / `uiStateWith` fixtures and the established `onAllNodesWithText("›").onLast()` chevron navigation; the `computeGapStats` test reuses `testEvent`. *Decoupling* — no production code touched; every new test calls a pure function with an explicit `ZoneId` argument (no `System.currentTimeMillis()`, no `android.*`). *Complexity* — three flat JVM tests + two Compose tests, none over ~25 lines, no `LaunchedEffect` / `remember` / coroutine scope. *Dead code* — new imports all referenced (`LocalDate` / `LocalDateTime` in `LogDetailViewModelTest`; `testEvent` / `ZoneId` in `InsightsEngineTest`; `assertCountEquals` in `BigPictureScreenTest`), `ktlintCheck`'s `no-unused-imports` green; no commented-out code, no TODOs, no self-updating tally added to a doc (TESTING.md edits are prose, no counts). *Repo hygiene* — `git status` shows only the 6 expected files (1 new test, 2 edited tests, 1 edited instrumented test, 2 docs); nothing secret-shaped, no local paths. The `2026-03-08` / `2026-11-01` DST anchors are fixed calendar facts (US DST = 2nd Sunday March / 1st Sunday November since 2007, baked into tzdata), not narrative timestamps — the tests take explicit instants and never read the wall clock, so they can't age; same pattern as `CalendarGridTest` / `VerdictEngineTest`. *Naming* — `CalendarMathTest.kt` mirrors `CalendarMath.kt` (as `CalendarGridTest` ↔ `CalendarGrid.kt`); domain tests use backtick-sentence names, `BigPictureScreenTest` uses `lowerCamel_withUnderscores` — both matched. No `Voice` keys added. *Hardcoded values* — `6 * MILLIS_PER_DAY` uses the named constant; DST dates discussed with the user and kept as fixed anchors. *Accessibility / Deprecated APIs* — no UI, no new API surface. *Spec Review* — no behaviour change; §9 already states the week-chevron dialog lists events "the same way" as the day dialog, and the new test confirms it rather than changing it.

**Deferred:** nothing.

**Docs updated:** `PROGRESS.md` — S10 section removed (resolved). `TESTING.md` — "Stats & visual data prep" row gains the direct `daysBetween` spring-forward / fall-back coverage and the `computeGapStats` DST carry-through; "ViewModels" row gains `computeEndedAt`'s fixed-24h-millis-across-a-spring-forward pin; "Compose UI — Big Picture" row notes the "lasted …" / "ongoing since …" assertion now runs in both the day and the week detail dialog. File-header KDoc on `CalendarMathTest.kt`.

**Tests:**
- New `CalendarMathTest` (3) — `daysBetween` counts calendar days, not fixed 24h chunks, directly across a spring-forward (asserts the elapsed millis are < 6 days, so a raw division would undercount) and a fall-back; and returns 0 for two instants on the same calendar day (the DST day itself).
- `InsightsEngineTest` +1 — `computeGapStats` with `zone = America/New_York` counts a past gap straddling the 2026-03-08 spring-forward as 7 calendar days, not 6.
- `LogDetailViewModelTest` +1 — `computeEndedAt` MANUAL "6 days" is exactly `occurredAt + 6 * MILLIS_PER_DAY`, and resolving that back in `America/New_York` lands the wall clock at 13:00 (an hour past the noon start) on the 13th — the by-design distortion pinned, not changed.
- `BigPictureScreenTest` +2 (instrumented, `@UiTest`) — a finished 3-day span and a still-running event each open the *week* detail dialog via the chevron and assert the span-range / "ongoing since" row copy renders once per covered day (`assertCountEquals(3)`), with the misleading clock time absent — mirroring the existing day-dialog pair.

**Verified:** `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green. Full unit suite 0 failures / 0 errors (`CalendarMathTest` 3/3, `InsightsEngineTest` 38/38, `LogDetailViewModelTest` 59/59). Instrumented `BigPictureScreenTest` 28/28 on `emulator-5554` (`Pixel_8_API36`), including the 2 new week-dialog tests. The `computeEndedAt` assertion failed on the first run against a wrong-direction expectation (11:00) — corrected to 13:00, since springing forward moves a fixed-millis interval *ahead* on the wall clock, and re-run green.

---

## feat/time-format-setting

**Scope:** PROGRESS.md's "Satellite · 12h/24h time format". Every clock time rendered 12-hour US (`h:mm a`, `Locale.US`) regardless of the device setting, from four separate `DateTimeFormatter.ofPattern` copies (`viewmodel/CaseDetailViewModel.kt`, `ui/bigpicture/BigPictureGrid.kt`, `ui/casedetail/InsightsTab.kt`; the spec also named `ui/share/ShareCardTemplate.kt`, which turned out to hold no time formatting at all). Added a two-value `TimeFormat` DataStore preference seeded from `DateFormat.is24HourFormat` until the user picks, an Appearance Settings row, a `LocalTimeFormat` CompositionLocal (mirroring `LocalVoice`), and consolidated the formatters into `viewmodel/EventTimeFormat.kt`.

**Decisions taken with the user before building (checklist "Spec Review"):**
- Preference is **two-value** (`TWELVE_HOUR` / `TWENTY_FOUR_HOUR`), **seeded from the device**: the DataStore key stays unset until the user picks, and every read with no stored value falls back to `DateFormat.is24HourFormat(context)`. Once picked it no longer tracks the device. (The alternative — a tri-state `FOLLOW_SYSTEM` default — was declined for the extra segmented chip.)
- Consolidation covers **all four `ofPattern` sites**, including `InsightsTab`'s locale-aware `formatFrequencyPeriodLabel` (moved verbatim, keeps its `locale` parameter).
- **No info ("i") icon** on the new row — "12-hour" / "24-hour" is self-explanatory, and an info icon would shift the positional indices `SettingsScreenTest`'s three info-icon tests depend on.

**Consolidation (checklist "Duplication" — the point of the task):**
- New `viewmodel/EventTimeFormat.kt` holds `formatEventTime` / `formatEventDate` / `formatEventTimeOfDay` (moved out of `CaseDetailViewModel.kt` — same package, so no import churn at the `CaseDetailScreen` / `LogDetailSheet` call sites or the two test files) plus `formatClockTime` / `formatSpanDate` / `formatMediumDate` / `formatWeekdayDayDate` (from `BigPictureGrid`'s two named + two inline formatters) and `formatFrequencyPeriodLabel` (from `InsightsTab`). Placed in the `viewmodel` package beside `formatMinutesDuration` (`OngoingEvent.kt`) — the established home for pure, Compose-free, JVM-testable presentation helpers, per `formatEventTime`'s own "split out … so it's unit-testable without Compose" KDoc.
- `formatFrequencyPeriodLabel` had a hardcoded `"Week of "` prefix — a user-visible chart-axis string outside the `Voice` layer (CLAUDE.md: "No inline UI strings, ever"). Since this diff relocates the function anyway, it's fixed here rather than left: new `Voice.insightsFrequencyWeekAxisLabel(date)` key ×3 (Plain/Bright "Week of …", Intense "The week of …", matching each voice's existing `bigPictureWeekDetailTitle`), passed in as a `weekOf: (String) -> String` lambda so `EventTimeFormat.kt` stays free of a `ui.voice` import. `FrequencyCard` gains a small `axisLabel` local so the two call sites stay readable. The month/day *date* formats (`"MMM d"` / `"MMM YYYY"`, numeric and locale-driven) aren't prose and stay in the shared util.
- `formatEventTime` builds its pattern from a `use24Hour` flag rather than string-swapping a sub-pattern: nine top-level `private val` formatters (year × {12,24}, time-only × {12,24}, span date, weekday-day, medium date), same style as the four it replaced.
- 24-hour uses `HH:mm` (zero-padded morning hour); 12-hour keeps `h:mm a`. `EVENT_TIME_ONLY_FORMATTER` was `ofLocalizedTime(SHORT)` before — switching to an explicit `h:mm a` pattern also removes the JDK-CLDR narrow-no-break-space quirk the old tests worked around with `contains` (the KDoc note is carried over).

**Wiring (checklist "Decoupling"):**
- `LocalTimeFormat` (`ui/theme/LocalTimeFormat.kt`, `staticCompositionLocalOf { TimeFormat.TWELVE_HOUR }`) provided once in `HodithApp` from a new `timeFormatFlow` param, fed from `MainActivity` via `settingsRepository.observeTimeFormat()` — mirrors the `LocalVoice` / decoration-local pattern exactly. Re-provided in `WidgetLogTrampolineActivity` (it hosts `LogDetailSheet`); the widget configure activities and both Glance widgets render no wall-clock times, confirmed by a source read, so they're untouched.
- Compose reads `LocalTimeFormat.current.is24Hour` at each site and passes a plain `Boolean` into the pure formatters — no UI type crosses into `viewmodel`. `LogDetailTimePickerDialog`'s `rememberTimePickerState` now takes `is24Hour` from the same source so the picker wheel matches the buttons.
- **No ViewModel formats clock times today** (source-read confirmed: `HomeViewModel` / `LogDetailViewModel` do date math, not display), so the spec's "VM-side via injected `SettingsRepository`" clause was moot — noted in the KDoc and PROGRESS for whoever adds the first such VM.
- `DataStoreSettingsRepository` gained an `@ApplicationContext Context` constructor param for `DateFormat.is24HourFormat`. That's a `data/`-layer class already coupled to `DataStore`/`Context` — not verdict/trigger/stats, no `domain/` purity rule in play. Its one framework-instantiated caller, `HodithBackupAgent` (constructs the repo directly, outside Hilt), was updated to pass `applicationContext`.

**Found & fixed (checklist walk-through against the real diff):**
- **New files were LF; the repo is CRLF** (no `.gitattributes`, every existing `.kt` is CRLF). `TimeFormat.kt` / `LocalTimeFormat.kt` / `EventTimeFormat.kt` / `EventTimeFormatTest.kt` were written LF-only — `ktlintCheck` / `lintDebug` / `test` all passed regardless, so nothing surfaced it until a manual `file` check on each new file vs an existing sibling. Converted all four to CRLF (`unix2dos`), re-ran `ktlintCheck --rerun-tasks` + `test` green. Edited files kept their CRLF (the Edit tool preserves existing endings).
- **`SettingsScreenTest`'s Data-plank action-row tests regressed on shorter test windows.** The new `TimeFormatSection` (one section-height, in the Appearance plank) pushed the Data plank's Export / Import / Delete-all rows down; each of those tests clicked its row with no `performScrollTo()`, so once a row was below the fold the click no-op'd and the confirm dialog never opened. The local full run (taller emulator window) caught only the two `deleteAllData_*` tests (bottom-most row); CI's `ui` shard (shorter window) then also failed the two `importButton_*` tests. Added `performScrollTo()` to all four (export included, one row from the edge), unconditionally safe. `SettingsScreenTest` 23/23 green locally; CI re-verifies the shorter-window case.

**Sections walked, nothing to do:** *Duplication* — the new abstract Voice keys (`settingsTimeFormatSectionLabel`, `insightsFrequencyWeekAxisLabel`) are in all three voices and diverge as `VoiceTest` requires; the two time-format option labels are shared interface-default `get() = "…"` keys, same as `themeOptionPlain` / `checkInIntervalOptionSeven`. No remaining inline UI strings. `SettingsViewModel`'s `combine` went 4→5 flows — still kotlinx.coroutines' typed 5-arg overload (a 6th forces the vararg form); `SettingsViewModelTest` exercises the new flow, green. *Complexity* — `TimeFormatSection` is ~15 lines, mirrors `ThemeSection` minus the info wrapper; `EventTimeFormat.kt` is flat top-level functions; nothing over ~150 lines, no `LaunchedEffect` / `remember` / coroutine-scope work. `SegmentedChoiceRow` (existing) is reused, not reimplemented; `rememberTimePickerState(is24Hour = …)` is current M3 API, not a hand-roll. *Dead code* — dropped now-unused `DateTimeFormatter` / `FormatStyle` imports from `CaseDetailViewModel`, `DateTimeFormatter` / `FormatStyle` / `Locale` from `BigPictureGrid`, `DateTimeFormatter` from `InsightsTab` (`ktlintCheck`'s `no-unused-imports` rule green confirms none missed); all params on the functions I touched (`EventDetailRow`'s `today` / `zoneId`, `TimeSection`'s args) stay referenced. No commented-out code, no TODOs, no self-updating tally added to a living doc. *Repo hygiene* — `git status` shows only the expected files (17 main incl. 3 new, 5 unit-test files incl. 1 new, 3 androidTest, 4 docs = 29); `git diff --numstat` per file matches the intended change size (no accidental large rewrites); nothing secret-shaped, no local paths, no build output. *Naming* — `EventTimeFormat.kt` / `LocalTimeFormat.kt` named after their primary symbol (the latter loosely matches `LocalCardDecorationStyle`-in-`CardDecorationStyle.kt` — the enum `TimeFormat` lives in `data/`, so the theme file only holds the local); formatter names follow the `formatX` family; test names follow each file's backtick-sentence style. *Hardcoded values* — no colours, no magic numbers; `"12-hour"` / `"24-hour"` are Voice defaults. *Accessibility* — `SegmentedChoiceRow` carries its own selection semantics (unchanged from the theme/check-in rows that already use it); no icon-only control added; the `is24Hour` picker change is a11y-neutral. *Deprecated APIs* — none introduced. *Spec Review* — §14's Appearance row and §6's event-row-timestamp clause updated to state the present ("12- or 24-hour per the Settings toggle"), not narrate the change.

**Deferred:**
- **Automated coverage of the device-clock seed** (`observeTimeFormat` falling back to `DateFormat.is24HourFormat` while the key is unset). No `DataStoreSettingsRepository` test exists for any setting (the repo is covered only indirectly via `FakeSettingsRepository`), and `DateFormat.is24HourFormat` needs a real `Context` — no Robolectric, no mocking library here, so it'd be an instrumented test for a one-line `if` wrapping an Android API whose fallback shape mirrors `observeTheme`. Not added to MANUAL_TEST_PLAN either — it doesn't cross a system-process boundary (that doc's scope), it just reads one system setting; a one-off on-device sanity check (fresh install on a 24-hour device shows the row on 24-hour) is enough and doesn't need recurring-journey real estate.

**Docs updated:** `HODITH_SPEC.md` §14 (Appearance row gains the 12h/24h toggle) and §6 (event-row timestamp notes the toggle). `TESTING.md` — new "Time formatting" unit-coverage row, Settings Compose-UI line gains the time-format selection + rendered-row assertion, backup manual-journey #7's excluded-prefs list gains "time format". `PROGRESS.md` — Satellite section replaced with a one-line done pointer, Story A intro bullet updated, B2's dependency list notes both new Voice keys (`settingsTimeFormatSectionLabel` + `insightsFrequencyWeekAxisLabel`) as landed. KDoc on `EventTimeFormat.kt`, `LocalTimeFormat.kt`, `TimeFormat.kt`, `TimeFormatSection`, and the new `insightsFrequencyWeekAxisLabel` key.

**Tests:**
- New `EventTimeFormatTest` (7 cases) — `formatClockTime` 12h/24h and zero-pad, `formatSpanDate` / `formatMediumDate` / `formatWeekdayDayDate`, `formatFrequencyPeriodLabel` per granularity.
- `CaseDetailFormattingTest` — `formatEventTime` calls updated to named `use24Hour` / `zone`; +2 cases (24-hour drops the AM/PM marker; year shows in both formats).
- `LogDetailViewModelTest` — `formatEventTimeOfDay` call updated; +1 case (`assertEquals("15:30", …)` for 24-hour).
- `SettingsViewModelTest` +2 — `uiState` reflects the persisted `TimeFormat`; `onTimeFormatSelect` persists it (mirrors the theme pair).
- `SettingsScreenTest` +2 — both options render; tapping "24-hour" invokes the callback with `TWENTY_FOUR_HOUR` (`@Smoke`, matching the theme/check-in picker smoke tests). The three positional info-icon tests are unaffected (no new icon). Also +`performScrollTo()` on the two `deleteAllData_*` tests — see "Found & fixed".
- `CaseDetailScreenTest` +1 — event row renders 24-hour time when `LocalTimeFormat provides TWENTY_FOUR_HOUR` (asserts against the shared formatter's output so it's device-zone-agnostic); the existing `formatEventTime` call site updated for the new signature; helper gains a `timeFormat` param.
- `FakeSettingsRepository` gains `timeFormat` (`MutableStateFlow`) + the two overrides — compiles against every ViewModel test that constructs it.
- `EventTimeFormatTest`'s `formatFrequencyPeriodLabel` case now passes a stub `weekOf` lambda and asserts it's applied only to the WEEK branch. `CaseDetailInsightsTabTest.frequencyGranularityToggle_day_switchesBucketLabelFormat` swapped its hardcoded `"Week of"` literal for `PlainVoice.insightsFrequencyWeekAxisLabel("").trim()`; re-run on the emulator, 21/21 green.

**Verified:** `./gradlew clean` then `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green (a one-off `lintAnalyze*` "bug in lint" failure on unrelated `backup/*Test` files, from stale KSP output after the emulator-crash churn, cleared by `clean`). `connectedDebugAndroidTest` on `emulator-5554` (`ANDROID_SERIAL`-scoped, DEV_PLAYBOOK §5): full suite 234/234 on the first pass after the `deleteAllData_*` fix; `SettingsScreenTest` 23/23 and `CaseDetailInsightsTabTest` 21/21 re-run green after the `importButton_*` and `insightsFrequencyWeekAxisLabel` fixes. `VoiceTest` + `EventTimeFormatTest` green. The local emulator crashed mid-suite (`INSTRUMENTATION_ABORTED: System has crashed`) three times over the session — host/AVD instability, cleared each time by a cold reboot, unrelated to the diff; CI's `ui` / `repository` shards run on a fresh disposable emulator and are the authoritative instrumented signal for the final state.

---

## feat/home-counts-duration-span

**Scope:** PROGRESS.md Story A item A9. `homeCaseRows` (`viewmodel/HomeViewModel.kt`) counted `events.count { it.occurredAt >= startOfToday }` / `>= startOfWeek` — pure start day — so a duration event started last week and stopped/logged today showed "Today 0 / This week 0" on its Home row, disagreeing with the calendar heatmap, which already credits every day an event's active span covers (A5/A7). Now both predicates test span reach (`endedAt`, or `now` for a running `START_STOP` event, or `occurredAt` for a point / non-tracking Case). The count stays a count of events, not active-days. Both widgets call `homeCaseRows` directly, so they inherit the fix. No schema change.

**Design decision resolved this pass (checklist "Spec Review"):** the day-attribution rule was raised with the user with worked examples. Chosen: an event counts once toward a window if its active span reaches into it (`todayCount` = "events active at some point today", `weekCount` = "events with any part of their span this week") — not a per-active-day sum, which would stop the Home number being a count of events. Written into spec §9 (active-span governed-surfaces sentence) and §14 (Home row).

**Found & fixed (checklist "Duplication" — ViewModel logic in more than one place):**
- **Extracted `activeSpanEnd(event, durationMode, now)`** into `viewmodel/OngoingEvent.kt`, beside `ongoingEventsIn` — spec §9's "where does this one event's active span end" rule, previously implemented once in `insightsTabState` (as a mode check + up-front `endedAt`-collapse + inner `spanEnd`) and about to be re-implemented in `homeCaseRows`. Both now call it. `insightsTabState` keeps its collapse (the domain stats fns — `computeGapStats`, `computeDurationStats` — read `endedAt` directly), so its inner `spanEnd` becomes a one-line delegate and `runsToNowWhenOpen` drops out; `InsightsTabStateTest` (30 cases) is green unchanged, confirming the refactor is behaviour-preserving. The earlier decision (A7 cleanup) to *not* give `insightsTabState` a mode-aware `spanEnd` was about avoiding three mode branches *in place of its one-line collapse* — it doesn't argue against a shared resolver, and A9's "`HomeViewModel.kt` only" note was a merge-ordering scope (A1–A8 now all landed), not a barrier to touching `InsightsTabState.kt`.
- `homeCaseRows` now inlines `activeSpanEnd(it, case.durationMode, nowMillis)` in the two `count {}` predicates — no local helper, no `tracksDuration` / `runsToNowWhenOpen` locals.

**Deferred:**
- **Instrumented coverage of the List widget's rendered count.** The Single-case widget renders the identical `homeCaseRows` count and *is* now covered (`WidgetActionsFlowTest.singleCaseWidget_todayCount_creditsADurationEventStillActiveToday` — a `MANUAL` event started two days ago, ended seconds ago, asserts the widget subtitle reads "Today: 1"). The List widget's rows are inside a Glance `LazyColumn` / `RemoteViewsService` `ListView` that never populates in an `AppWidgetHost` test (documented in `WidgetActionsFlowTest`'s KDoc and DEV_PLAYBOOK §5), so its count stays a manual check — added as MANUAL_TEST_PLAN Widgets item 7.
- **Future-dated `occurredAt` (> now).** Undefined before and after this change; not introduced here.

**Sections walked, nothing to do:** *Decoupling* — production side is clean: `homeCaseRows` / `activeSpanEnd` stay pure top-level functions taking `now: Long`, no `System.currentTimeMillis()`, no `android.*`; `activeSpanEnd` reaches for `data.tracksDuration` (module-`internal`, the pattern every visual surface and `NotificationEvaluator` already use); no UI type. The new *instrumented* test does read `System.currentTimeMillis()` directly — it has to, because `WidgetActionsFlowTest` runs the widget against the real injected clock, not a fake — anchored so its span lands in "today" with a one-minute margin (flaky only within a minute of local midnight; the count math itself is JVM-covered, this only proves the render). *Complexity* — `activeSpanEnd` is one expression; `homeCaseRows` loses a local `fun` and two `val`s (net −3 there); `insightsTabState` loses a `val` and its `spanEnd` body shrinks to a delegate; nothing over ~150 lines, no composable / `LaunchedEffect` / `remember` / coroutine scope touched. *Dead code* — `DurationMode` import dropped from `InsightsTabState.kt` (now only `case.durationMode` property access + `.tracksDuration`); `data.tracksDuration` import added to `OngoingEvent.kt`, dropped from `HomeViewModel.kt` (the local that used it is gone); no commented-out code, no TODOs (the `// MUTATION TEST` marker used during verification was removed), no self-updating tally in a living doc. *Repo hygiene* — `git status` shows only the expected files (3 main, 3 test, 5 docs); nothing secret-shaped, no local paths; files stayed CRLF (`ktlintCheck` passed after one blank-line fix, no `ktlintFormat` run). *Naming* — `activeSpanEnd` matches the `datesCovered` / `spansMultipleDays` span-helper family it's documented to pair with; test names follow each file's `backtick sentence` style. *Hardcoded values* — the instrumented test's time offsets now use `MILLIS_PER_DAY` / `MILLIS_PER_MINUTE` (matching `OngoingEventTest`) rather than raw `n * 24 * 60 …`; nothing else. *Accessibility / Deprecated APIs* — no UI, no new API surface. *Spec Review* — §9's added clause explains the rule's effect ("a run stopped and logged today still shows on its Home row"), in the same explanatory register as the paragraph's existing midnight-crossing example, not as a changelog of prior behaviour.

**Docs updated:** `HODITH_SPEC.md` §9 (Home's today/this-week counts added to the active-span governed-surface list, with the "counted once, not per active day" clarifier) and §14 (Home row parenthetical). `TESTING.md` ViewModels row (`homeCaseRows`' counts folded into the existing span-collapse clause alongside `bigPictureUiState` / `insightsTabState` / `eventDetailSummary`). `MANUAL_TEST_PLAN.md` Widgets item 7 (List widget duration-event count), items 8–12 renumbered. `PROGRESS.md` (A9 → one-liner in the renamed A1–A9 "done" block; full section removed; intro line updated; A9 struck from the chain intro). `OngoingEvent` KDoc on `activeSpanEnd`; `HomeViewModel` inline comment.

**Tests:**
- `HomeViewModelMappingTest` +8 (20 total). Six pin the new rule: a running `START_STOP` span from before the week and from earlier today; a `MANUAL` event started earlier but ended today; one that ended earlier this week (week yes, today no); a `NONE` Case's stored `endedAt` collapsing to a point (0/0); a `MANUAL` end-less event staying a point. Two close pre-existing gaps in the same file while it's open: a multi-day running span counted once alongside two same-day point events (`todayCount == 3`, not per-day), and two Cases mapped in one call with counts not bleeding across rows. The five existing point-event boundary tests are unchanged and stay green (for a `NONE` fixture `activeSpanEnd == occurredAt`, so the predicate is identical to the old one).
- `OngoingEventTest` +4 — `activeSpanEnd` per branch: stored `endedAt` for a finished tracking event, `now` for a running `START_STOP` one, `occurredAt` for a `MANUAL` end-less one, and `occurredAt` for a `NONE` Case whatever `endedAt` is stored.
- `WidgetActionsFlowTest` +1 (instrumented, Single-case widget count — see Deferred).
- `InsightsTabStateTest` unchanged, 30/30 green — the extraction is behaviour-preserving there.
- Mutation checks: reverting the two `homeCaseRows` predicates to `it.occurredAt >= …` fails 4 of the 8 new `HomeViewModelMappingTest` cases (the three "started earlier, reaches in" cases + the counted-once multi-day case; the other 4 guard the `tracksDuration` gate, the `MANUAL`-no-`endedAt` branch, and multi-Case mapping). Reverting `activeSpanEnd` to `event.occurredAt` outright fails the new instrumented test on the emulator.

**Verified:** `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green. Unit suite green (`HomeViewModelMappingTest` 20/20, `OngoingEventTest` 19/19, `InsightsTabStateTest` 30/30, `HomeViewModelTest`, `DurationModeTest`). Instrumented `WidgetActionsFlowTest` 4/4 on the `Pixel_8_API36` emulator (`ANDROID_SERIAL` scoped — a physical Pixel 8 was also attached and fails all `AppWidgetHost` tests on the OEM `grantbind` restriction per DEV_PLAYBOOK §5; the full `connectedDebugAndroidTest` had the same 8 pre-existing widget-bind failures until `adb shell appwidget grantbind` was run on the emulator, then green). Mutation-checked: reverting `activeSpanEnd` to `event.occurredAt` fails the new instrumented test on the emulator. Manual on-device check of the List widget count and the running-span-crosses-midnight case left to the human (MANUAL_TEST_PLAN item 7).

---

## feat/duration-switch-in-conversion — A1–A8 test-coverage audit

**Scope:** a whole-cluster audit of the automated coverage for Story A items A1–A8 (Start/Stop, duration & ongoing-event polish) against `HODITH_SPEC.md` §§6/9/10/11/14/15 — checking that main paths / edge cases / error scenarios are covered, that tests fail when the behaviour breaks (not passing for the wrong reason), that setup isn't duplicated, and that what the tests assert matches the spec. Landed on this branch as seven commits grouped by nature (fixture share → three bug fixes → assertion strengthening → two coverage passes → docs).

**Found & fixed — production defects the audit surfaced:**
- **`CaseEditViewModel` held-conversion flags never reset (spec §6:151).** `stopRunningOnSave` / `convertOpenEndedOnSave` latched cumulatively, and the leave/enter checks keyed off the unsaved in-memory `durationMode`. `NONE→START_STOP→NONE` with both toggles confirmed before a single `save()` stamped `endedAt = now` on a days-old point — a multi-day span from a no-op round-trip. Fixed: reset both flags on every `onDurationModeChange`, and key the transition checks off the Case's *persisted* mode so an unsaved intermediate `START_STOP` can't be "left". +2 regression tests (mutation-checked).
- **`NotificationEvaluator.silenceAnchorFor` ignored `durationMode` (spec §9/§10/§11).** A Case now in `NONE` with a stored `endedAt` (kept across `MANUAL → NONE`, §6) measured `SILENT_FOR` / check-in silence from `endedAt`, later than the `occurredAt` the spec mandates — every visual surface already collapses this, the notification path was the lone hold-out. Fixed via the shared `DurationMode.tracksDuration` extension, reusing the existing `getMostRecentEventForCase` (which the `fix/duration-gap-from-end` cleanup had flagged as unused-by-production — now un-orphaned). +2 `NotificationEvaluatorTest` cases + a stronger Fake test (W6) + a DAO case; mutation-checked.
- **`computeGapStats` didn't floor a reversed `endedAt`.** Line 77 took `event.endedAt ?: event.occurredAt` with no floor, unlike `datesCovered` / `spansMultipleDays` which both `maxOf(start, end)`. A bad stored `endedAt < occurredAt` (from an old round-trip, §6:151 acknowledges these exist) dragged the running reach below the event's own start. Fixed with a `maxOf(..., event.occurredAt, ...)`; +2 tests; mutation-checked.
- **Small `draftFrom` polish:** a zero-length or reversed stored span now leaves the duration-amount field blank rather than showing `"0"` / `"-1"`. +3 tests.

**Found & fixed — tests that passed for the wrong reason (checklist "changed methods making existing tests pass for the wrong reason"):**
- Three `InsightsTabStateTest` cases (`gaps display reads the current gap from a finished duration event's end`, `heatmap shades a still-running event through today`, `a still-running event that began before today makes the Case multi-day`) used `eventAtDay(0)` as filler — a bare `endedAt == null` event that on a `START_STOP` Case is *itself* ongoing, tripping the `eventActiveNow` short-circuit / spanning to `now` and masking the behaviour under test. New `finishedPoint(day)` fixture (`durationEvent(day, day)`) fills that "not the running one" role; W2 also switched from `containsAll` to set-equality. Verified: `gaps display reads the current gap…` now fails if the end-anchored reach is reverted — it didn't before.
- `CaseEditViewModelTest`'s two "held, not applied" checks asserted `events.all { endedAt == null }` on a seed where every event was already `endedAt == null` — tautological. Now seed a finished event alongside and compare the full `{id → endedAt}` map pre/post.
- `CaseDetailFormattingTest`'s duration-line tests only ever exercised `tracksDuration = true` (the default) except one; folded the redundant pair into an explicit `isOngoing = true, tracksDuration = false` compose-the-suppressors case.
- `FakeHodithRepositoryTest.getLatestEventEndForCase` couldn't catch an impl that ignored still-open rows (only the DAO test could, and every `NotificationEvaluatorTest` runs on the Fake) — added the open-row-is-the-max case.

**Found & fixed — spec behaviour with no test at all:**
- `DurationMode.tracksDuration` extension (new `DurationModeTest`); `insightsTabState`'s duration-stats card gate (absent for `NONE`, present otherwise) on the JVM; Trend counts unchanged by a multi-day event's span; the `NONE↔MANUAL` transition-contract rows (no dialog, stored `endedAt` kept); the leave-`START_STOP` path's "finished events untouched" and its `→ MANUAL` target (only `→ NONE` was asserted); `runningCount == 0` for a non-`START_STOP` Case with end-less events at the row level; a check-in reading zero silence while an event runs (only `SILENT_FOR` was covered); the silence anchor with several concurrent open events; the widget subtitle's `runningCount ≥ 2` count branch; `saveLogSheetEvent` starting a second concurrent event on a running `START_STOP` Case.

**Found & fixed — test hygiene (checklist "Duplication"):**
- ~9 unit-test files re-rolled the full `CaseEntity` / `EventEntity` literal, and `millisAtDay` / `millisAt` / `durationEvent` / `eventAtDay` were copy-pasted identically across the domain and viewmodel test files. Extracted `app/src/test/kotlin/com/secondmonday/hodith/testsupport/` — a `Fixtures` object (the full-literal `case()` / `event()` builders, namespaced so a class can keep a thin clock-defaulted `private fun testCase(...) = Fixtures.case(...)` forwarder without shadowing) plus top-level `testCase` / `testEvent` / `runningEvent` / `finishedEvent` / `durationEvent` / `eventAtDay` / `finishedPoint` shorthands and `TEST_ZONE` / `millisAtDay` / `millisAt`. Migrated 16 files; net −138 lines on that commit; the whole unit suite stayed green with identical assertions. `fix/duration-gap-from-end`'s note that "the codebase already duplicates … per unit-test file by house style, so the duplication is fine" is retired (per the user — that convention held only while there were few test files); the note itself is left in that historical entry as written.
- `WidgetActionsFlowTest` drives real Views but carries no `@UiTest`, so it runs in the `repository` CI shard rather than `ui`. Left as-is (harmless — `grantbind` runs for both shards) with a one-line note in TESTING.md so it isn't mistaken for a missing tag.

**Divergences flagged for a decision (checklist "Spec Review"):** both raised with the user and resolved this pass rather than deferred — the silence-anchor one by fixing the code (option chosen over amending the spec), the §6 "log header always reads as a count ('1 running' included)" wording by rewording the spec to "the ongoing indicator (the pill + elapsed for one, a running count for several)", since the single-event header renders the pill, not literal "1 running" text. No open A1–A8 divergence remains.

**Deferred:** DST-boundary tests for `computeEndedAt` day-scaling / `daysBetween` gap counting, and week-detail-dialog spanned-row instrumented coverage — low likelihood, logged as PROGRESS.md **S10** (`test/duration-edge-coverage`) rather than widened into this pass.

**Sections walked, nothing to do:** *Decoupling* — `silenceAnchorFor` stays in `notification/` and reaches for `data.tracksDuration` (module-`internal`, same pattern the visual surfaces use); `computeGapStats` still takes `now`/`zone` as parameters, no `System.currentTimeMillis()`; `CaseEditViewModel` gains no `android.*`; no UI type crosses a layer. *Complexity* — `onDurationModeChange` gained a 3-line reset preamble and swapped `state.durationMode` for `existingCase?.durationMode` in two checks (net +7, reads clearer); `draftFrom` gained one `.takeIf { it > 0L }`; no composable touched, no `LaunchedEffect`/`remember`. *Dead code* — unused imports trimmed from every migrated test file; `getMostRecentEventForCase` un-orphaned. *Repo hygiene* — `git status` clean between commits; every file `ktlintFormat` rewrote to LF was re-normalised to CRLF (per the sed/CRLF memo); nothing secret-shaped, no local paths. *Naming* — `testsupport` mirrors the `data`/`domain` package convention; `Fixtures` / `finishedPoint` / `durationEvent` are descriptive; new Voice keys — none added. *Hardcoded values* — none; `500_000L` and friends are test literals, `endedAt = occurredAt` is the spec's zero-length definition. *Accessibility / Deprecated APIs* — no UI, no new API surface (pre-existing Moshi/kapt warning only).

**Docs updated:** `HODITH_SPEC.md` §6 (log-header wording), §11 (`SILENT_FOR` and check-in anchor both note the non-tracking-Case `occurredAt` collapse). `TESTING.md` (Notification evaluation, Stats & visual data prep, ViewModels, Widgets — chrome & trampoline rows). `DEV_PLAYBOOK.md` §5 (physical-device `grantbind` can report success yet `bindAppWidgetIdIfAllowed` still fail). `PROGRESS.md` (new Standalone item S10). `CaseEditViewModel` / `InsightsEngine` / `NotificationEvaluator` / `LogDetailViewModel` inline comments on each fix.

**Verified:** per commit, `ktlintCheck → lintDebug → test → assembleDebug` sequential (test-only commits ran `ktlintCheck → testDebugUnitTest`). Full unit suite green throughout. Instrumented: `EventDaoTest` (17/17) and `WidgetActionsFlowTest` (3/3, including the new count-pill assertion) on `Pixel_8_API36` — the same classes fail identically on the attached physical device on the pre-existing `grantbind` issue now noted in DEV_PLAYBOOK §5. D1/D2/D3 + the W1 fixture fix + the `draftFrom` change each confirmed to fail their new test when reverted (`QA_AUDIT_RULES.md` §2 mutation discipline).

---

## feat/duration-switch-in-conversion

**Scope:** PROGRESS.md Story A item A8. `CaseEditViewModel.onDurationModeChange` only intercepted *leaving* `START_STOP`; entering it fell straight through with no event changes, so every pre-existing `endedAt == null` event (one-tap / quick logs on a `NONE` Case, duration-less `MANUAL` events) was instantly reinterpreted as a live ongoing span — "N running" on Home and Case Detail, per-row Stop, stale-prompt fire, Big Picture trails-to-today, inflated heatmap/streak, a "never silent" check-in anchor. Switching back out then stamped `endedAt = now` on all of them, turning an old point into a multi-day span. Now the enter-`START_STOP` transition mirrors the leave one: a confirm dialog when open-ended events exist, and on confirm `save()` stamps `endedAt = occurredAt` on each (an instant event at its own start, not `now`). Stored `endedAt` is otherwise untouched; no schema change.

**Found & fixed:**
- The enter path is a structural mirror of the existing leave path — `showEnterStartStopConfirm` beside `showLeaveStartStopConfirm`, `confirmEnterStartStop` / `dismissEnterStartStop` beside their leave twins, `convertOpenEndedOnSave` beside `stopRunningOnSave`.
- **Checklist "does ViewModel logic appear in more than one place?" caught the `save()` event-stamping loop:** the first draft repeated `observeEventsWithTagsForCase(id).first().filter { endedAt == null }.forEach { updateEvent(copy(endedAt = …)) }` verbatim for the leave and enter branches, only the stamped value differing. Extracted `private suspend fun stampOpenEndedEvents(caseId, endedAt: (EventEntity) -> Long)`; leave calls it with `{ stoppedAt }`, enter with `{ it.occurredAt }`. Unlike A6/A7's span-end resolvers (kept parallel because the data shapes differed), here both sites are the identical list/filter/update over `EventWithTags` — the lambda is the only real difference.
- The switch-*in* stamp uses the event's own `occurredAt`, not `clock.nowMillis()` — the whole point of the item is that an old point stays a point. The leave block keeps `now` because a genuinely-running event is being ended. Both branches are guarded by a `state.durationMode ==/!=` check so the two never both run.
- `runningEventCount`'s KDoc widened: it counts every `endedAt == null` event in any mode and now gates both dialogs. Not renamed — under `START_STOP` "running" is still exact, and a rename would churn the screen plus both test files for a cosmetic gain.
- Leave-`START_STOP` body copy reworded in all three voices to say *why* events stop now ("Only Start/Stop tracks a running event, so switching away stops all N of them now …") — the old wording just stated the fact.
- Spec §6 draft said "…round-tripped before this contract existed…" — trimmed the history-narrating clause (checklist "do living docs narrate how something changed") to a present-tense limitation: no migration exists for an inflated `endedAt`, fix by editing the event.

**Deferred:**
- `convertOpenEndedOnSave` / `stopRunningOnSave` are never reset to `false` after `save()`. Pre-existing pattern for the leave flag; both are inert once set because of the mode guard, and `save()` on this screen is terminal (navigates away on `isSaved`). Left matching the existing code rather than adding resets to both.

**Sections walked, nothing to do:** *Duplication* — 4 new Voice keys (`enterStartStopConfirmTitle` / `Action` / `CancelAction` + `enterStartStopConfirmBody`) authored in all three voices this commit; the dialog reuses `ConfirmDialog` (a third call site of the same shape as the archive and leave confirms — the established pattern in this file, not worth a wrapper). *Decoupling* — `CaseEditViewModel` stays free of `android.*`; `stampOpenEndedEvents` takes its time value as a lambda so the enter path needs no clock at all; no UI type crosses in; the new helper is a `private suspend fun` called only inside the existing `viewModelScope.launch`. *Complexity* — `onDurationModeChange` gained one `if` block; `save()` is now two one-line calls; no composable past ~150 lines; no `LaunchedEffect` / `remember` touched (the dialog is driven by `uiState`, like the leave one). *Dead code* — one new import (`EventEntity`, for the helper's lambda param); no commented-out blocks, no TODOs, no self-updating tallies added to living docs. *Repo hygiene* — `git status` shows only the 9 expected files (3 main, 1 unit-test, 1 instrumented-test, 4 docs); nothing secret-shaped, no local paths; `CaseEditViewModelTest.kt` re-normalised to CRLF after `ktlintFormat` rewrote it LF. *Naming* — every new symbol mirrors its `leave`/`stopRunning` counterpart; `stampOpenEndedEvents` matches the `*Events` verb-noun style. *Hardcoded values* — none; `endedAt = occurredAt` is the zero-length definition, already spec'd. *Accessibility* — reuses `ConfirmDialog`; no icon buttons, no colour-only cue, no tap-target change. *Deprecated APIs* — none new (`lintDebug` clean bar the pre-existing Moshi-kapt warning).

**Docs updated:** `HODITH_SPEC.md` §6 (new "Duration-mode transition contract" subsection with the `NONE`/`MANUAL`/`START_STOP` transition table and the `occurredAt`-vs-`now` rationale; the inline leave-`START_STOP` sentence folded into a pointer to it), §14 (New/edit Case row notes the confirm fires in either direction, cancel leaves the mode unchanged). `CaseEditViewModel` `runningEventCount` KDoc, `stampOpenEndedEvents` KDoc, and the two inline comments on the leave/enter branches in `save()`. `PROGRESS.md` (A8 → done one-liner in the renamed A1–A8 block; full section removed; chain intro trimmed — A9/A10 are independent, not a chain). `TESTING.md` (ViewModels row + Compose UI row extended to name the enter-`START_STOP` confirm). No `DEV_PLAYBOOK.md` / `MANUAL_TEST_PLAN.md` change (nothing crosses a process boundary).

**Verified:** `ktlintCheck` → `lintDebug` → `test` → `assembleDebug`, sequential, all green. Unit: `CaseEditViewModelTest` (+10 — enter holds behind the confirm; confirm converts to `endedAt = occurredAt` on save, not `now`; dismiss keeps the mode and touches nothing; `MANUAL` end-less events convert too; already-finished events untouched; no-open-ended and no-events skip the dialog; `NONE → START_STOP → NONE` round-trip keeps a point a point; dismiss-then-reenter re-prompts) plus the 5 existing leave-`START_STOP` tests still green. `VoiceTest` green (reflection-walks the 4 new keys). Instrumented `CaseEditScreenTest` run on `Pixel_8_API36` (16 tests, 0 failed) — +3: the enter-`START_STOP` dialog renders naming the count, confirm/cancel wiring, neither dialog shown when both flags clear.
