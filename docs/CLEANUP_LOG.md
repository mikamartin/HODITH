...
**Verified:** `ktlintCheck → lintDebug → test → assembleDebug` sequential, all green; `ktlintCheck --rerun-tasks` + `test` re-run green after the LF→CRLF conversion. Full `connectedDebugAndroidTest` on `emulator-5554` (`ANDROID_SERIAL`-scoped; a physical Pixel 8 also attached — DEV_PLAYBOOK §5), 234/234 green after the `deleteAllData_*` fix. (First full run flagged those 2; a mid-suite `INSTRUMENTATION_ABORTED: System has crashed` on a follow-up run was emulator instability — cleared by a cold reboot, unrelated to the diff.) `unit` suite `SettingsViewModelTest` / `CaseDetailFormattingTest` / `LogDetailViewModelTest` / `EventTimeFormatTest` / `VoiceTest` all green.
